...
/**
  ...
  */
abstract class DFDesign(implicit ctx: DFDesign.Context)
    extends DFDesign.Abstract {
  private[DFiant] final lazy val __ctx: DFDesign.Context = ctx
  private[DFiant] final lazy val __portArgs: Map[DFAny, DFAny] = {
    val ret = Map(
      ctx.args.value.flatMap(l =>
        l.flatMap {
          case (name, x: DFAny) =>
            Some(x -> DFAny.Port.In(x.dfType).setName(name))
          case _ => None
        }
      ): _*
    )
...
abstract class MetaDesign(lateConstruction: Boolean = false)(implicit
    meta: Meta
) extends DFDesign()(new MetaDesign.Context(meta)) {
  final def plantMember[T <: DFMember](member: T): T =
    __db.plantMember(this, member)
  ...
  final protected implicit val __lateConstructionConfig
      : LateConstructionConfig = LateConstructionConfig.Force(lateConstruction)
}

object MetaDesign {
  class Context(meta: Meta)
      extends DFBlock.Context(
        meta,
        implicitly[Meta.SymbolOf[MetaDesign]],
        null,
        ASIS,
...
object DFDesign {
  ...
  trait Abstract extends DFOwner.Container {
    ...
    private[DFiant] final val owner: DFDesign.Block =
      DFDesign.Block.Internal(this)(designType, inlinedRep, simMode)(__ctx)
    ...
    final protected implicit def __contextOfDefs[T <: String with Singleton](
      ...
      new ContextOf[T](
        meta,
        Meta.SymbolOf(valueOf[T]),
        this,
        ASIS,
        __db,
        new ClassArgs[T] { val value = List() }
      ) {
        def newInterface(updatedCtx: ContextOf[T]): Any = ???
      }
    final protected implicit def __contextOfDesign[T <: DFDesign](implicit
        meta: Meta,
        symbol: Meta.SymbolOf[T],
        args: ClassArgs[T]
    ): ContextOf[T] =
      new ContextOf[T](meta, symbol, this, ASIS, __db, args) {
        def newInterface(updatedCtx: ContextOf[T]): Any = ???
      }
  ...
  final implicit class DesignOps[D <: Abstract](dsn: D) {
    ...
    def enable()(implicit ctx: DFAny.Context): D with FSM.Capable = {
      assert(
        !dsn.isTop,
        "Stalling/Enabling the top-level design is not allowed"
      )
...
//    DFString.Op.Frontend
  ...
  sealed trait Block extends DFBlock {
    ...
    def headerCodeString(implicit printer: CSPrinter): String =
      s"trait $designType extends DFDesign"
  }
  object Block {
    final case class Internal(
        ...
        tags: DFMember.Tags,
        inlinedRep: Option[DFInlineComponent.Rep]
    ) extends Block {
    ...
    object Internal {
      def apply(container: DFOwner.Container)(
          designType: String,
          inlinedRep: Option[DFInlineComponent.Rep],
          simMode: DFSimDesign.Mode
      )(implicit
          ctx: Context
      ): Block =
        ctx.db.addContainerOwner(
          container,
          if (ctx.container == null) Top(designType, ctx.meta, simMode)
          else Internal(designType, ctx.owner, ctx.meta, inlinedRep)
        )
      object Unref {
        def unapply(internal: Internal)(implicit
            getSet: MemberGetSet
        ): Option[
          (String, DFOwner, DFMember.Tags, Option[DFInlineComponent.Rep])
        ] = {
          import internal._
  ...
  final case class Control(
      designRef: Control.DesignRef,
      op: Control.Op.Entry,
      ownerRef: DFOwner.Ref,
      tags: DFMember.Tags
  ) extends DFAny.CanBeAnonymous
      with CanBeGuarded {
    protected[DFiant] def =~(
        that: DFMember
    )(implicit getSet: MemberGetSet): Boolean =
  ...
  object Control {
    def apply(design: Block, op: Op.Entry)(implicit
        ctx: DFAny.Context
    ): Control = {
    ...
    object DesignRef {
      ...
      def unapply(ref: DFMember.Ref): Boolean =
        ref.refType match {
          case _: Type => true
          case _       => false
        }
...
//      sealed abstract class Status(implicit meta : Meta) extends Entry with Product with Serializable
  ...
  final case class DB(
      ...
      globalTags: Map[(Any, ClassTag[_]), DFMember.CustomTag]
  ) { self =>
    ...
    def concat(db: DB): DB =
      DB(
        this.members ++ db.members.drop(1),
        this.refTable ++ db.refTable,
        this.globalTags ++ db.globalTags
      )
    ...
    lazy val connectionTable: Map[DFAny.Member, DFNet] =
      members.collect {
        case n: DFNet if n.isConnection => (n.toRef.get, n)
      }.toMap

    lazy val connectionTableInverted: Map[DFAny.Member, List[DFNet]] =
      members
        .collect { case n: DFNet if n.isConnection => n }
        .groupBy(n => n.fromRef.get)
    ...
    def getConnectionFrom(v: DFAny.Member): List[DFNet] =
      connectionTableInverted.getOrElse(v, List())

    def getAssignmentsTo(v: DFAny.Member): Set[DFAny.Member] =
      assignmentsTable.getOrElse(v, Set())
    def getAssignmentsFrom(v: DFAny.Member): Set[DFAny.Member] =
      assignmentsTableInverted.getOrElse(v, Set())
    ...
    //then its owner is set to None.
    private lazy val arrTypes
        : Map[DFVector.Type[_ <: DFAny.Type, _], Option[DFDesign.Block]] =
      members.foldLeft(
        Map.empty[DFVector.Type[_ <: DFAny.Type, _], Option[DFDesign.Block]]
      ) {
        case (arrMap, arrMember @ DFVector(_, _)) => //an arr member
          val arrType =
            arrMember.dfType.asInstanceOf[DFVector.Type[_ <: DFAny.Type, _]]
          if (arrMember.isPort)
            arrMap + (arrType -> None) //IO means a global arr type
          else
            arrMap.get(arrType) match {
              case Some(Some(owner)) => //arr type already found
                if (owner == arrMember.getOwnerDesign)
                  arrMap //same design block -> nothing to do
                else
                  arrMap + (arrType -> None) //used in more than one block -> global arr type
              case Some(None) => arrMap //known to be a global type
              case None =>
                arrMap + (arrType -> Some(
                  arrMember.getOwnerDesign
                )) //found new arr type
            }
        case (arrMap, _) => arrMap //not an arr member
      }
    private lazy val invertedArrTypes = arrTypes.invert
    lazy val getGlobalArrTypes: Set[DFVector.Type[_ <: DFAny.Type, _]] =
      invertedArrTypes.getOrElse(None, Set())
    private lazy val localArrTypes
        : Map[DFDesign.Block, Set[DFVector.Type[_ <: DFAny.Type, _]]] =
      invertedArrTypes.flatMap {
        case (Some(b), set) => Some(b -> set)
        case _              => None
      }
    def getLocalArrTypes(
        design: DFDesign.Block
    ): Set[DFVector.Type[_ <: DFAny.Type, _]] =
...
//      members.collectFirst{case n : DFNet.Connection if n.toRef.get == v => n.fromRef.get}
    ...
    @tailrec private def OMLGen[O <: DFOwner: ClassTag](
      ...
      globalMembers match {
        case m :: mList
            if getOwnerFunc(m) == localOwner => //current member indeed belongs to current owner
          val updatedStack1 =
            (localOwner -> (m :: localMembers)) :: updatedStack0
          m match {
            case o: O
                if classTag[O].runtimeClass.isInstance(
                  o
    ...
    lazy val ownerMemberTable: Map[DFOwner, List[DFMember]] = Map(
      ownerMemberList: _*
    )
    ...
    //holds a hash table that lists members of each owner block. The member list order is maintained.
    lazy val designMemberTable: Map[DFDesign.Block, List[DFMember]] = Map(
      designMemberList: _*
    )
    ...
    @nowarn(
      "msg=The outer reference in this type test cannot be checked at run time"
    )
    private final case class ReplacementContext(
      ...
      def changeRef(
          origRef: DFMember.Ref,
          updateMember: DFMember
      ): ReplacementContext = {
          ...
          memberTable.get(origMember) match {
            case Some(refs) =>
              ...
              val actualReplacement =
                this.memberRepTable.getOrElse(repMember, repMember)
              ReplacementContext(
                refFilter(refs).foldLeft(refTable)((rt2, r) =>
                  rt2.updated(r, actualReplacement)
                ),
    ...
    def patch(patchList: Iterable[(DFMember, Patch)]): DB =
      if (patchList.isEmpty) this
      else {
        val patchTable = patchList
          .flatMap {
            ...
            case (_, Patch.Replace(_, Patch.Replace.Config.ChangeRefOnly, _)) =>
              None
            ...
            case (
                  m,
                  Patch.Replace(_, Patch.Replace.Config.ChangeRefAndRemove, _)
                ) =>
              Some((m, Patch.Remove))
            ...
            case (
                  owner: DFOwner,
                  Patch.Add(db, Patch.Add.Config.InsideFirst)
                ) =>
            ...
            case (
                  ...
                  Patch.Add(
                    db,
                    Patch.Add.Config.After | Patch.Add.Config.InsideLast
                  )
                ) =>
            ...
            case (m, Patch.Move(movedMembers, config)) =>
              val modMove = (m, config) match {
                ...
                case (
                      owner: DFOwner,
                      Patch.Move.Config.After | Patch.Move.Config.InsideLast
                    ) =>
                  owner.getVeryLastMember match {
                    case Some(l) =>
                      (l, Patch.Move(movedMembers, Patch.Move.Config.After))
                    case None =>
                      (
                        (
                          owner,
                          Patch.Move(movedMembers, Patch.Move.Config.After)
                        )
                      )
                  }
                case (m, Patch.Move.Config.Before) =>
                  (m, Patch.Move(movedMembers, config))
                case _ => ???
              }
          ...
          .foldLeft(Map.empty[DFMember, Patch]) {
            case (tbl, (m, p)) if tbl.contains(m) =>
              (tbl(m), p) match {
                ...
                case (Patch.Add(db1, config1), Patch.Add(db2, config2))
                    if (config1 == config2) =>
                  tbl + (m -> Patch.Add(db1 concat db2, config1))
                //concatenating addition and move with the same configuration
                case (
                      Patch.Add(db, addConfig),
                      Patch.Move(movedMembers, moveConfig)
                    ) if (addConfig == moveConfig) =>
                  tbl + (m -> Patch.Move(
                    db.members.drop(1) ++ movedMembers,
                    moveConfig
                  ))
                case (
                      Patch.Move(movedMembers, moveConfig),
                  ...
                  tbl + (m -> Patch.Move(
                    movedMembers ++ db.members.drop(1),
                    moveConfig
                  ))
          ...
          patchTable.get(m) match {
            ...
            case Some(Patch.Add(db, config)) =>
              val notTop =
                db.members.drop(
                  1
                ) //adding the members without the Top design block
              config match {
                case Patch.Add.Config.After  => m :: notTop
                case Patch.Add.Config.Before => notTop :+ m
                case Patch.Add.Config.ReplaceWithFirst(
                      Patch.Replace.Config.ChangeRefOnly,
                      _
                    ) =>
                  m :: notTop
                case Patch.Add.Config
                      .ReplaceWithLast(Patch.Replace.Config.ChangeRefOnly, _) =>
                  notTop :+ m
            ...
            case None =>
              Some(m) //not in the patch table, therefore remain as-is
          }
        )
        ...
        val patchedRefTable = patchList
          .foldLeft(ReplacementContext.fromRefTable(refTable)) {
            ...
            case (rc, (origMember, Patch.Add(db, config))) =>
              ...
              val actualNewOwner = rc.memberRepTable.getOrElse(
                newOwner,
                newOwner
              ) //owner may have been replaced before
              val dbPatched = db.patch(
                db.top -> Patch
                  .Replace(actualNewOwner, Patch.Replace.Config.ChangeRefOnly)
              )
              val repRT = config match {
                case Patch.Add.Config.ReplaceWithFirst(_, refFilter) =>
                  val repMember =
                    db.members(
                      1
                    ) //At index 0 we have the Top. We don't want that.
                  rc.replaceMember(origMember, repMember, refFilter)
                ...
                case Patch.Add.Config.Via =>
                  val repMember =
                    db.members.last //The last member is used for Via addition.
                  rc.replaceMember(
                    origMember,
                    repMember,
              ...
              val updatedPatchRefTable = dbPatched.refTable.view
                .mapValues(m => rc.memberRepTable.getOrElse(m, m))
            ...
            case (rc, (origMember, Patch.Remove)) =>
              memberTable.get(origMember) match {
                case Some(refs) =>
                  rc.copy(refTable =
                    refs.foldLeft(rc.refTable)((rt2, r) => rt2 - r)
                  )
                case None => rc
              }
            case (
                  rc,
                  (_, Patch.ChangeRef(origMember, refFunc, updatedRefMember))
                ) =>
    ...
    def setGlobalTags(
        tagList: List[((Any, ClassTag[_]), DFMember.CustomTag)]
    ): DB =
      copy(globalTags = globalTags ++ tagList)
    def patch(singlePatch: (DFMember, Patch)): DB = patch(List(singlePatch))
    @tailrec private def getGuards(
        currentOwner: DFBlock,
        targetOwner: DFBlock,
        currentGuards: List[DFAny.Member]
    ): List[DFAny.Member] =
      currentOwner match {
        case _: DFDesign.Block => currentGuards //reached the design block
        case o: DFBlock if o == targetOwner =>
          currentGuards //can't go past the target owner
        case cb: DFConditional.IfElseBlock =>
          getGuards(
            cb.getOwnerBlock,
            targetOwner,
            cb.condRefOption.map(c => c.get).toList ++ currentGuards
          )
        case cb: DFConditional.CaseBlock =>
          getGuards(
            cb.getOwnerBlock,
            targetOwner,
            cb.matchHeaderRef.matchValRef.get :: currentGuards
          )
      }
    ...
    lazy val consumerDependencyTable: Map[DFAny.Member, Set[DFAny.Member]] = {
      ...
      members.foldLeft(netPass)((dt, m) =>
        m match {
          case f: DFAny.Func2 =>
            dt + (f -> Set(f.leftArgRef.get, f.rightArgRef.get))
          case a: DFAny.Alias =>
            val idxOption: Option[DFAny.Member] = a match {
              case asel: DFAny.Alias.ApplySel => Some(asel.idxRef.get)
              case _                          => None
            }
            ...
            (dt.get(a), dt.get(relVal)) match {
              ...
              case (Some(aDeps), Some(relDeps)) =>
                dt + (a -> (aDeps + relVal)) + (relVal -> (relDeps + a ++ idxOption))
              case (Some(aDeps), None) =>
                dt + (a -> (aDeps + relVal)) + (relVal -> (Set(a) ++ idxOption))
              //the alias is just read, and therefore it's dependent only on the rel and possible idx of ApplySel
              case (None, _) => dt + (a -> (Set(relVal) ++ idxOption))
            }
          case _ => dt
        }
      )
    ...
    lazy val producerDependencyTable: Map[DFAny.Member, Set[DFAny.Member]] = {
      consumerDependencyTable.foldLeft(
        Map.empty[DFAny.Member, Set[DFAny.Member]]
      ) {
        case (dt, (consumer, producerSet)) =>
    ...
    def printConsumerDependencyTable: DB = {
      println(
        consumerDependencyTable
          .map(op =>
            s"${op._1.getFullName} <- ${op._2.map(e => e.getFullName)}"
          )
          .mkString("\n")
      )
      this
        ...
        members
          .map {
            ...
            case m =>
              ...
              s"${m.name}_${m.hashCode().toHexString} : ${m.typeName}   ->   ${owner.name}_${owner
                .hashCode()
                .toHexString} : ${owner.typeName}"
          }
          .mkString("\n")
      )
      this
  ...
  object DB {
    ...
    object Patch {
      ...
      object Replace {
        ...
        trait RefFilter {
          def apply(refs: Set[DFMember.Ref])(implicit
              getSet: MemberGetSet
          ): Set[DFMember.Ref]
        }
        object RefFilter {
          ...
          object All extends RefFilter {
            def apply(refs: Set[DFMember.Ref])(implicit
                getSet: MemberGetSet
            ): Set[DFMember.Ref] = refs
          }
          //Only references from outside the given block are replaced
          final case class Outside(block: DFDesign.Block.Internal)
              extends RefFilter {
            def apply(
                refs: Set[DFMember.Ref]
            )(implicit getSet: MemberGetSet): Set[DFMember.Ref] =
              refs.collect {
                case r: DFMember.OwnedRef
                    if r.owner.get.isOutsideOwner(block) =>
                  r
              }
          }
          ...
          final case class Inside(block: DFDesign.Block) extends RefFilter {
            def apply(
              ...
              refs.collect {
                case r: DFMember.OwnedRef if r.owner.get.isInsideOwner(block) =>
                  r
              }
          }
        }
      ...
      object Add {
        def apply(design: MetaDesign, config: Config): Add =
          Add(design.getDB, config)
        ...
        def apply(addedMember: DFMember, config: Config): Add =
          Add(List(addedMember), config)

        sealed trait Config extends Product with Serializable {
          def ==(moveConfig: Move.Config): Boolean =
            (this, moveConfig) match {
        ...
        object Config {
          ...
          final case class ReplaceWithFirst(
              replacementConfig: Replace.Config =
                Replace.Config.ChangeRefAndRemove,
              refFilter: Replace.RefFilter = Replace.RefFilter.All
          ) extends Config
      ...
      final case class Move private (
          movedMembers: List[DFMember],
          config: Move.Config
      ) extends Patch
      object Move {
        def apply(owner: DFOwner, config: Config)(implicit
            getSet: MemberGetSet
        ): Move =
    ...
    class Mutable { self =>
      ...
      private var members
          : mutable.ArrayBuffer[(DFMember, Set[DFMember.Ref], Boolean)] =
        mutable.ArrayBuffer()
      def top: Block.Top =
        members.head._1 match {
          case m: Block.Top => m
        }
...
//          println(f"""${"injecting"}%-20s ${newContainer}""")
        ...
        private def enqContainerOwner(
            container: DFOwner.Container,
            owner: DFOwner
        ): Unit = {
          stack = OwnershipContext(
            container,
            owner,
            injectedContainer,
            fsmTrack,
            fsmDuringElaboration
          ) :: stack
          fsmTrack = ListSet()
          fsmDuringElaboration = false
...
//          println(f"""${"exitContainer"}%-20s ${exitingContainer.nameAndType}%-30s ${container.nameAndType}""")
        ...
        def exitAllContainers(): Unit =
          while (!duringExitContainer && stack.nonEmpty) deqContainerOwner()

        def getCurrentOwner(container: DFOwner.Container): DFOwner = {
          checkContainerExits(container)
          stack.head.owner
        }
        def injectOwnerAndRun[T](
            container: DFOwner.Container,
            injectedOwner: DFOwner
        )(block: => T): T = {
//          println("injecting", injectedOwner)
      ...
      ///////////////////////////////////////////////////////////////
      object global_tags {
        private[Mutable] var tagMap
            : mutable.Map[(Any, ClassTag[_]), DFMember.CustomTag] =
          mutable.Map()
        def set[CT <: DFMember.CustomTag: ClassTag](
            taggedElement: Any,
            tag: CT
        ): Unit =
          tagMap += ((taggedElement, classTag[CT]) -> tag)
        def get[CT <: DFMember.CustomTag: ClassTag](
            taggedElement: Any
        ): Option[CT] =
          tagMap.get((taggedElement, classTag[CT])).asInstanceOf[Option[CT]]
      }

      def addContainerOwner[O <: DFOwner](
          container: DFOwner.Container,
          owner: O
      ): O = {
      ...
      def addMember[M <: DFMember](
          member: M
      )(implicit ctx: DFMember.Context): M =
        addMember(ctx.container, member)

      def addMemberOf[M <: DFMember](
          member: DFMember
      )(implicit ctx: DFMember.Context): M with DFMember.RefOwner =
      ...
      def plantMember[M <: DFMember](
        ...
        newRefFor[DFOwner, DFOwner.Ref.Type, DFOwner.Ref](
          member.ownerRef,
          container.owner
      ...
      def hasToConnectionFor(dfVar: DFAny.Member): Boolean =
      ...
      def getMember[M <: DFMember, T <: DFMember.Ref.Type, M0 <: M](
          ref: DFMember.Ref.Of[T, M]
      ): M0 = refTable(ref).asInstanceOf[M0]
      def setMember[M <: DFMember](
          originalMember: M,
          newMemberFunc: M => M
      ): M = {
