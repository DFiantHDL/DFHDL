  ...
  def connectWith(that: DFOwner)(implicit ctx: DFBlock.Context): Unit = {
    ...
    (leftMembers lazyZip rightMembers).foreach {
      case (left: DFAny.Dcl, right: DFAny.Dcl) if left.dfType == right.dfType =>
        left.connect(right)
      case (left: DFInterface.Owner, right: DFInterface.Owner) =>
        left.connectWith(right)
      case (left: DFScope.Owner, right: DFScope.Owner) =>
        left.connectWith(right)
      case (left: DFAny.Dcl, right: DFAny.Dcl) if left.dfType != right.dfType =>
...
object DFOwner {
  trait Container
      extends HasTypeName
      with DFDesign.Frontend
      with DisallowExternalExtensions {
    type Owner <: DFOwner
    ...
    final private[DFiant] lazy val __parent: Container =
      if (__ctx.container == null) this else __ctx.container
    final def isTop: Boolean = __parent == this
    @tailrec private def isInsideParent(
        thisContainer: Container,
        thatContainer: Container
    ): Boolean = {
  ...
  final implicit class AbstractExt[T <: DFOwner](owner: T) {
    def getMembers(implicit getSet: MemberGetSet): List[DFMember] =
      getSet.getMembersOf(owner)
    def getMembersAtAnyHierarchy(implicit
        getSet: MemberGetSet
    ): List[DFMember] =
      getSet.designDB.members
        .dropWhile(_ != owner)
        .takeWhile(m => m.isInsideOwner(owner) || m == owner)
    def getVeryLastMember(implicit getSet: MemberGetSet): Option[DFMember] = {
      import getSet.designDB
      val last = owner match {
        case block: DFDesign.Block =>
          designDB.designMemberTable(block).lastOption
      ...
      last match {
        ...
        case Some(o: DFOwner) =>
          o.getVeryLastMember match {
            case None =>
              Some(o) //found empty owner as last member ==> return the owner
  ...
  object NameFlatten {
    object UnderscoreSuffix extends NameFlatten {
      def apply(memberName: String, ownerName: String): String =
        s"${ownerName}_${memberName}"
    }
    object NoSuffix extends NameFlatten {
      def apply(memberName: String, ownerName: String): String =
        s"${ownerName}${memberName}"
    }
...
abstract class ContextOf[T](
    val meta: Meta,
    val symbol: Meta.SymbolOf[T],
    val container: DFOwner.Container,
    val dir: DFDir,
    val db: DFDesign.DB.Mutable,
    val args: ClassArgs[T]
) extends DFMember.Context { self =>
  def newInterface(updatedCtx: ContextOf[T]): Any
  final def updateDir(updatedDir: DFDir): ContextOf[T] =
    new ContextOf[T](meta, symbol, container, updatedDir, db, args) {
      override def newInterface(updatedCtx: ContextOf[T]): Any =
