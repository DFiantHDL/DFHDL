...
object DFSInt extends DFAny.Companion {
  ...
  // Public Constructors
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  def apply[W](checkedWidth : SIntWidth.Checked[W])(implicit ctx : DFAny.Context) : DFAny.NewVar[Type[W]] =
  ...
  final case class Token(width : Int, value : Option[BigInt]) extends DFAny.Token.Of[Type[Int], BigInt] { left =>
    ...
    def +^ (right : Token) : Token = mkTokenS(right, _ + _, (left.width max right.width) + 1)
    def -  (right : Token) : Token = mkTokenS(right, _ - _, left.width max right.width)
    def -^ (right : Token) : Token = mkTokenS(right, _ - _, (left.width max right.width) + 1)
    def *  (right : Token) : Token = mkTokenS(right, _ * _, left.width max right.width)
    def *^ (right : Token) : Token = mkTokenS(right, _ * _, left.width + right.width)
  ...
  object Token {
    ...
    trait Implicits {
      implicit def __DFSIntTokenAsIsInt[LW, V <: Int, OW](
        implicit
      ...
      implicit def __DFSIntTokenAsIsLong[LW, V <: Long, OW](
        implicit
      ...
      implicit def __DFSIntTokenAsIsBigInt[LW, OW]
      ...
      implicit def __DFSIntTokenToFit[LW, V, RW](
        implicit
  ...
  object Op {
    ...
    class AbleOps[L](value : L) extends Able[L](value) {
      ...
      final def +^  [RW](right : DFSInt[RW])(implicit op: `Op+^`.Builder[L, true, DFSInt[RW]]) = op(left, right)
      final def -   [RW](right : DFSInt[RW])(implicit op: `Op-`.Builder[L, false, DFSInt[RW]]) = op(left, right)
      final def -^  [RW](right : DFSInt[RW])(implicit op: `Op-^`.Builder[L, true, DFSInt[RW]]) = op(left, right)
    ...
    trait Implicits {
      ...
      sealed class __DFSIntFromXInt[L <: XInt](left : L) extends AbleOps[ValueOf[L]](new ValueOf(left))
      ...
      sealed class __DFSIntFromXLong[L <: XLong](left : L)(implicit di : DummyImplicit) extends AbleOps[ValueOf[L]](new ValueOf(left))
      ...
      final implicit class __DFSIntOps[LW](val left : DFSInt[LW]){
        ...
        def unary_- (implicit op: `Op-`.Builder[0, true, DFSInt[LW]]) = op(0, left)
  ...
  protected abstract class `Ops+Or-`[Op <: Func2.Op.Negateable](val op : Op) {
    ...
    object Builder {
      ...
      implicit def evConst_op_DFSInt[L, LW, LE, RW](
        implicit
        ctx : DFAny.Context,
        lConst : DFAny.Const.AsIs.Aux[Type[RW], L, _ <: Type[LW]],
