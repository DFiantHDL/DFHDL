...
final case class ModelsimSimulation[D <: DFSimDesign, B <: BackendStage](
  ...
  private val vcomPath =
    if (modelsimDir.isEmpty) "vcom" else s"$modelsimDir\\vcom"
  private val vlogPath =
    if (modelsimDir.isEmpty) "vlog" else s"$modelsimDir\\vlog"
  private val vlibPath =
    if (modelsimDir.isEmpty) "vlib" else s"$modelsimDir\\vlib"
  private val vsimPath =
    if (modelsimDir.isEmpty) "vsim" else s"$modelsimDir\\vsim"
  private val mkWorkCmd: String = s"$vlibPath $workPath"
  private def vhdlCompileCmd(revisionCmd: String): String =
    s"$vcomPath -work $workPath $revisionCmd ${fileNameSeq.mkString(" ")}"
  private val verilogCompileCmd: String =
    s"$vlogPath -work $workPath ${fileNameSeq.mkString(" ")}"
  private val runsimCmd: String =
    s"""$vsimPath -lib $workPath $topName -t 1ps -do "set NumericStdNoWarnings 1" -do "run -all""""
...
object ModelsimSimulation {
  ...
  implicit def verilogSupport[R <: verilog.Revision](implicit
      revision0: R
  ): SupportedBackend[verilog.Backend[R]] =
    new SupportedBackend[verilog.Backend[R]] {
      val backend: verilog.Backend[R] = new verilog.Backend[R] {
        val revision: R = revision0
      }
    }
  implicit def vhdlSupport[R <: vhdl.Revision](implicit
      revision0: R
  ): SupportedBackend[vhdl.Backend[R]] =
    new SupportedBackend[vhdl.Backend[R]] {
