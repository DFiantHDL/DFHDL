...
final case class GHDLSimulation[D <: DFSimDesign, R <: Revision](
    ...
    programFile: String =
      "ghdl",                 //default: assumes the program 'ghdl' is in path
  ...
  private val flags =
    s"$workDirFlag -frelaxed-rules --ieee=synopsys --std=$std $userFlags"
  ...
  def runToFile(fileName: String): this.type = {
    initSim

    {
      s"$programFile -r $flags $topEntity --ieee-asserts=disable" #> new java.io.File(
        fileName
      )
    }.!
    this
