...
sealed abstract class FSM(implicit protected[FSM] val ctx: DFBlock.Context) {
  ...
  /**
    ...
    * @return a new connected FSM
    */
  final def ==>[T <: FSM.Capable](dstBlock: => T)(implicit
      ctx: DFBlock.Context,
      ta: FSM.HasFSMAscription,
      di: DummyImplicit
...
object FSM {
  trait Capable
  protected[DFiant] final class Step private[FSM] (
      val alwaysBlock: () => Unit,
      val exitBlock: () => Unit = () => {}
  )(implicit
      ctx: DFBlock.Context
  ) extends FSM {
    ...
    protected[FSM] def addDstStep(dstStep: Step): Unit =
      dstSteps = dstSteps + dstStep
  ...
  protected[FSM] final class StepChain private[FSM] (
      stepMap: ListMap[Step, Option[Step]],
      nextFSM: () => FSM
  )(implicit
      ctx: DFBlock.Context
  ) extends FSM {
    ...
    def =^>(exitBlock: => Unit)(implicit
      ...
      val updatedStepMap =
        stepMapRemovedLast.updated(
          stepMapRemovedLast.last._1,
          Some(updatedStep)
        ) + (updatedStep -> None)
  ...
  def apply[T](alwaysBlock: => T)(implicit
      ctx: DFBlock.Context,
      ta: HasFSMAscription
  ): FSM = new Step(() => alwaysBlock).track

  private def prevStep(implicit ctx: DFAny.Context): FSM.Step =
    ctx.db.getPrevFSMStep match {
  ...
  private implicit class StepMapOps(list: List[(Step, Set[Step])]) {
    ...
    def fixOrder: List[(Step, Set[Step])] = {
      ...
      if (sourceOnlySteps.size > 1)
        throw new IllegalArgumentException(
          s"Found more than one starting step:\n${sourceOnlySteps.mkString(", ")}"
        )
  ...
  private implicit class FSMTableOps(table: ListMap[Step, Set[Step]]) {
    ...
    def genNames: (String, ListMap[Step, String]) = {
      val prefix = "(.*)_(.*)".r
      val headName =
        steps.map(s => s.ctx.meta.name.toString).longestCommmonPrefix match {
          case prefix(s, _) if s.nonEmpty => s
          case _                          => "fsm"
        }
      val nameGroups = steps.zipWithIndex.groupByOrdered(k =>
        k._1.ctx.meta.name.toString.stripPrefix(s"${headName}_")
      )
      ...
      if (nameGroups.size == 1)
        (
          headName,
          ListMap.from(steps.zipWithIndex.map {
            case (s, i) => s -> s"S${suffixGen.format(i)}"
          })
        )
      else
        (
          headName,
          ListMap.from(nameGroups.flatMap {
            case (name, (head, i) :: Nil) =>
              if (name.isEmpty) Some(head -> s"S${suffixGen.format(i)}")
              else Some(head              -> name)
            case (name, steps) =>
              steps.map {
                case (s, i) => s -> s"${name}_${suffixGen.format(i)}"
              }
          })
        )
    }
  }
  protected[DFiant] def elaboration(
      mutableDB: DFDesign.DB.Mutable,
      fsmTrack: List[FSM]
  ): Unit = {
        ...
        f.getStepMap.map {
          case (step, nextStep) =>
            ...
            nextStep match {
              ...
              case Some(ns)
                  if !step.getDstSteps
                    .contains(ns) && step.getDstSteps.nonEmpty =>
                throw new IllegalArgumentException(
...
//    println(fsmTrack.map(_.getStepMap.map(_._1.getDstSteps)))
    ...
    val mixedSteps: List[(Step, Set[Step])] =
      fsmTrack.flatMap(_.getStepMap.keys.map(k => (k, k.getDstSteps)))
...
//    println("distinctStepSets", distinctStepSets)
    val fsmTables: Seq[ListMap[Step, Set[Step]]] = mixedSteps
      .groupByOrdered {
        case (step, _) => distinctStepSets.find(group => group.contains(step))
      }
      .map(e =>
        ListMap.from(e._2.toList.fixOrder)
      ) //.map{e => ListMap(e._2.toSeq : _*)).fixOrder}.toList
//    println("fsmTables", fsmTables)
    ...
    fsmTables.foreach { fsmTable =>
      ...
      object states
          extends EnumType.Auto()(ctx.meta.setName(s"${fsmName}_states"))
      val entries: ListMap[Step, states.Entry] =
        stepNames.map(e => e._1 -> states.Entry()(ctx.meta.setName(e._2)))
      val state =
        DFEnum(states) init (entries(steps.head)) setName (s"${fsmName}_state")
      ...
      steps.foldLeft[
        DFConditional.NoRetVal.HasCaseDF[DFEnum.Type[states.type], true]
      ](matchHeader) {
        case (pm, step) =>
          pm.casedf(entries(step)) {
