...
final case class BackendEmitter(
    seq: Seq[Either[BackendEmitter.Ref, String]],
    backendStage: BackendStage,
    ownerRef: DFOwner.Ref,
    tags: DFMember.Tags
) extends CanBeGuarded
    with CanBeAnonymous {
  override protected[DFiant] def =~(
      that: DFMember
  ...
  def codeString(implicit printer: CSPrinter): String =
    backendStage.codeString + "\"" + seq.collect {
...
object BackendEmitter {
  type Ref = DFAny.Ref.ConsumeFrom
  def apply(seq: Seq[Either[DFAny.Member, String]], backendStage: BackendStage)(
    ...
    implicit lazy val ret: BackendEmitter with DFMember.RefOwner =
      ctx.db.addMemberOf[BackendEmitter](
        BackendEmitter(refSeq, backendStage, ctx.owner, ctx.meta)
      )
