...
final case class DFNet(
  ...
  def codeString(implicit printer: CSPrinter): String = {
    ...
    if (isConnection && hasLateConstruction) {
      if (toRef.getOwner == this.getOwner)
        s"$toRefString $opString$CMT/*<--*/ $fromRefString"
...
object DFNet {
  ...
  def apply(to: DFAny.Member, op: Op, from: DFAny.Member)(implicit
      ctx: Context
  ): DFNet = {
  ...
  object Assignment {
    def apply(to: DFAny.Member, from: DFAny.Member)(implicit
        ctx: Context
    ): DFNet = DFNet(to, Op.Assignment, from)

    def unapply(arg: DFNet)(implicit
        getSet: MemberGetSet
    ): Option[(DFAny.Member, DFAny.Member, DFOwner, DFMember.Tags)] =
      arg match {
        case DFNet(toRef, Op.Assignment, fromRef, ownerRef, tags) =>
  ...
  object Connection {
    ...
    def unapply(arg: DFNet)(implicit
      ...
      arg match {
        case DFNet(
              toRef,
              Op.Connection | Op.LazyConnection,
              fromRef,
              ownerRef,
              tags
            ) =>
  ...
  object Inlined {
    def unapply(arg: DFNet)(implicit getSet: MemberGetSet): Boolean =
      arg match {
        case net: DFNet if net.isConnection =>
          net.toRef.get.getOwnerBlock match {
