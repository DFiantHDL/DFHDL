...
object DFVector extends DFAny.Companion {
  final case class Type[T <: DFAny.Type, N](
      cellType: T,
      cellNum: TwoFace.Int[N]
  ) extends DFAny.Type {
    type CellNum                              = N
    ...
    val width: TwoFace.Int[Width] =
      TwoFace.Int.create[Width](cellType.width * cellNum)
    ...
    def getTokenFromBits(fromToken: DFBits.Token): DFAny.Token = {
      ...
      val cells =
        for (i <- 0 until cellNum)
          yield fromToken.bitsWL(cellType.width, i * cellType.width)
      Token(cellType, cells.toVector)
    }
    def assignCheck(from: DFAny.Member)(implicit ctx: DFAny.Context): Unit =
      from match {
        case r @ DFVector(cellType, cellNum)
            if cellType == this.cellType && cellNum == this.cellNum.getValue =>
      }
    ...
    override def equals(obj: Any): Boolean =
      obj match {
        case Type(cellType, cellNum) =>
          this.cellType == cellType && this.cellNum.getValue == cellNum.getValue
        case _ => false
      }
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Public Constructors
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  def unapply(arg: DFAny.Member): Option[(DFAny.Type, Int)] =
    arg.dfType match {
  ...
  final case class Token(cellType: DFAny.Type, value: Vector[DFAny.Token])
    ...
    def ==(that: Token): DFBool.Token =
      DFBool.Token(logical = true, this.value equals that.value)
    def !=(that: Token): DFBool.Token =
      DFBool.Token(logical = true, !(this.value equals that.value))
  ...
  object Token {
    ...
    implicit def bubbleOfDFType[T <: DFAny.Type, N]
        : DFAny.Token.BubbleOfDFType[Type[T, N]] =
    ...
    type ToFit[LT <: DFAny.Type, LN, V] =
      DFAny.Token.ToFit.Summon.SAM[Type[LT, LN], Vector[V], TokenN[LT, LN]]
    type AsIs[LT <: DFAny.Type, LN, V] =
      DFAny.Token.AsIs.Summon.SAM[Type[LT, LN], Vector[V], TokenN[LT, LN]]
    trait Implicits {
      implicit def __DFVectorTokenVectorToFit[LT <: DFAny.Type, LN, V](implicit
        ...
        (from, value) => {
          ...
          val tokenVec: Vector[DFAny.Token] =
            value.map(tokenOf(from.cellType, _))
          Token(from.cellType, tokenVec).typeTag[Type[LT, LN]]
        }
      implicit def __DFVectorTokenVectorAsIs[LT <: DFAny.Type, LN, V](implicit
  ...
  object Op {
    trait Implicits {
      final implicit class __DFVectorOps[LT <: DFAny.Type, LN](
          val left: DFVector[LT, LN]
      ) {
        def ===[RN](right: DFVector[LT, RN])(implicit
            op: `Op===`.Builder[DFVector[LT, LN], DFVector[LT, RN]]
        ) = op(left, right)
        def =!=[RN](right: DFVector[LT, RN])(implicit
            op: `Op=!=`.Builder[DFVector[LT, LN], DFVector[LT, RN]]
        ) = op(left, right)
//        def ++  [RN](right : DFVector[LT, RN])(implicit op: `Op++`.Builder[DFVector[LT, LN], DFVector[LT, RN]]) = op(left, right)
      ...
      final implicit class __DFVectorAliases[
          LT <: DFAny.Type, LN, Mod <: DFAny.Modifier
      ](val left: DFAny.Value[Type[LT, LN], Mod]) {
        def apply[I, W](idx: Exact[I])(implicit
            ctx: DFAny.Context,
            w: BitsWidthOf.IntAux[LN - 1, W],
            op: DFAny.`Op:=,<>`.Builder[DFUInt.Type[W], I]
        ): DFAny.Value[LT, Mod] =
          DFAny.Alias.ApplySel
            .fromArray(left, op(DFUInt.Type(w(left.dfType.cellNum - 1)), idx))
      }
    }
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  object `Op:=,<>` {
    ...
    trait Implicits {
      implicit def __DFVector_op_DFVector[T <: DFAny.Type, LN, RN](implicit
          ctx: DFAny.Context,
          checkLWvRW: `LN == RN`.CheckedShell[LN, RN]
      ): Builder[Type[T, LN], DFVector[T, RN]] =
        (left, right) => {
  ...
  object `LN == RN` extends Checked1Param.Int {
    ...
    type Msg[LN, RN] =
      "This operation does not permit applying different element numbers. Found: LHS-width = " + ToString[
        LN
      ] + " and RHS-width = " + ToString[RN]
    type ParamFace = Int
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  protected abstract class OpsCompare[Op <: Func2.Op](op: Op)(
    ...
    object Builder {
      implicit def evDFVector_op_DFVector[T <: DFAny.Type, LN, RN](implicit
          ctx: DFAny.Context,
          checkLWvRW: `LN == RN`.CheckedShell[LN, RN]
      ): Builder[DFVector[T, LN], DFVector[T, RN]] =
        (left, right) => {
  ...
  object `Op++` {
    ...
    object Builder {
      ...
      object Inference {
        ...
        type ON[LN, RN, ResN] =
          TwoFace.Int.Shell2Aux[CalcL, LN, Int, RN, Int, ResN]
      }

      implicit def evDFBits_op_DFBits[T <: DFAny.Type, LN, RN, ON](implicit
          ctx: DFAny.Context,
          oN: Inference.ON[LN, RN, ON]
      ): Builder.Aux[DFVector[T, LN], DFVector[T, RN], DFVector[T, ON]] =
        new Builder[DFVector[T, LN], DFVector[T, RN]] {
          ...
          def apply(left: DFVector[T, LN], right: DFVector[T, RN]): Out = {
            ...
            val out = DFAny.Func2(
              Type(left.dfType.cellType, oCellNum),
              left,
              DFAny.Func2.Op.++,
              right
            )(_ ++ _)
