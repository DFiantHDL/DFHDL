...
object DFBool extends DFAny.Companion {
  final case class Type(logical: Boolean) extends DFAny.Type {
    ...
    def codeString(implicit printer: CSPrinter): String =
      if (logical) s"${printer.config.TP}DFBool()"
      else s"${printer.config.TP}DFBit()"
  }
  ...
  def apply()(implicit ctx: DFAny.Context): DFAny.NewVar[Type] =
    DFAny.NewVar(Type(logical = true))
  ...
  implicit def toBooleanError(dfbool: DFBool): Boolean =
    macro toBooleanErrorMacro
  ...
  final case class Token(logical: Boolean, value: Option[Boolean])
      extends DFAny.Token.Of[Type, Boolean] { left =>
    val width              = 1
    ...
    def &&(right: Token)(implicit bb: Bubble.Behaviour): Token = {
      ...
      (left.value, right.value, bb) match {
        ...
        case (Some(false), None, Bubble.DontCare) =>
          Token(logical, value = false)
        case (None, Some(false), Bubble.DontCare) =>
          Token(logical, value = false)
    ...
    def ||(right: Token)(implicit bb: Bubble.Behaviour): Token = {
      ...
      (left.value, right.value, bb) match {
        ...
        case (None, Some(false) | None, Bubble.DontCare) =>
          Token.bubble(logical)
      }
    }
    ...
    def select[ST <: DFAny.Token](thenSel: ST, elseSel: ST)(implicit
        bb: Bubble.Behaviour
    ): ST = {
  ...
  object Token {
    implicit val bubbleOfToken: DFAny.Token.BubbleOfToken[Token] = t =>
      Token.bubble(t.logical)
    implicit val bubbleOfDFType: DFAny.Token.BubbleOfDFType[DFBool.Type] = t =>
      Token.bubble(t.logical)
    def apply(value: Int): Token =
      value match {
        case 0 => Token(logical = false, value = false)
        case 1 => Token(logical = false, value = true)
      }
    def apply(logical: Boolean, value: Boolean): Token =
      Token(logical, Some(value))
    def bubble(logical: Boolean): Token = Token(logical, None)

    type Summon[V] = DFAny.Token.Exact.Summon.SAM[Type, V, TokenB]
    trait Implicits {
      implicit def __DFBoolTokenInt[V <: Int](implicit
      ...
      implicit def __DFBoolTokenBoolean[V <: Boolean]: Summon[V] =
        (from, value) => {
  ...
  // Match Pattern
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  class Pattern(set: Set[Boolean])
      extends DFAny.Pattern.OfSet[Type, Boolean, Pattern](set) {
    protected def matchCond(matchVal: DFAny.Of[Type], value: Boolean)(implicit
        ctx: DFAny.Context
    ): DFBool = {
  ...
  object Pattern extends PatternCO {
    ...
    object Able {
      implicit class DFBoolPatternBoolean[R <: Boolean](val right: R)
          extends Able[R] {
    ...
    object Builder {
      implicit def ev[LW]: Builder[Type] =
        new Builder[Type] {
          def apply[R](left: Type, right: Seq[Able[R]]): Pattern = {
            val patternSet = right
              .map(e => e.bool)
              .foldLeft(Set.empty[Boolean])((set, bool) => {
  ...
  object Arg {
    implicit def ev[R](implicit
        conv: DFAny.`Op:=,<>`.Builder[DFBool.Type, R]
  ...
  object Op {
    ...
    class AbleOps[L](value: L) extends Able(value) {
      ...
      /**
        ...
        */
      final def ||(right: DFBool)(implicit op: `Op||`.Builder[L, DFBool]) =
        op(left, right)
    ...
    trait Implicits {
      ...
      final implicit def __DFBoolFrom0(left: 0): __DFBoolFrom0 =
        new __DFBoolFrom0(left)
      sealed class __DFBoolFrom1(left: 1) extends AbleOps[1](left)
      final implicit def __DFBoolFrom1(left: 1): __DFBoolFrom1 =
        new __DFBoolFrom1(left)
      sealed class __DFBoolFromBoolean[L <: Boolean](left: L)
          extends AbleOps[L](left)
      final implicit def __DFBoolFromBoolean[L <: Boolean](
          left: L
      ): __DFBoolFromBoolean[L] = new __DFBoolFromBoolean[L](left)
      sealed class __DFBoolFromDFBitsW1[LW](left: DFBits[LW])(implicit
          ctx: DFAny.Context,
          r: Require[LW == 1]
      ) extends AbleOps[DFBool](DFAny.Alias.AsIs(Type(logical = false), left))
      final implicit def __DFBoolFromDFBitsW1[LW](left: DFBits[LW])(implicit
          ctx: DFAny.Context,
          r: Require[LW == 1]
      ...
      implicit class __DFBoolOps(val left: DFBool) {
        ...
        /**
          * @return the dataflow logical Or result.
          */
        final def ||[R](right: Exact[R])(implicit
            op: `Op||`.Builder[DFBool, R]
        ) = op(left, right)
        ...
        /**
          * @return the dataflow logical Xor result.
          */
        final def ^[R](right: Exact[R])(implicit op: `Op^`.Builder[DFBool, R]) =
          op(left, right)
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  protected abstract class BoolOps[Op <: Func2.Op](op: Op)(
      func: (Token, Token) => DFBool.Token
  ) {
    @scala.annotation.implicitNotFound(
      "Operation is not supported between type ${L} and type ${R}"
    )
    trait Builder[-L, -R] extends DFAny.Op.Builder[L, R] { type Out = DFBool }

    object Builder {
      def create[L, R](properLR: (L, R) => (DFBool, DFBool))(implicit
          ctx: DFAny.Context
      ): Builder[L, R] =
        (leftL, rightR) => {
      ...
      implicit def evDFBool_op_DFBool[L <: DFBool, R <: DFBool](implicit
          ctx: DFAny.Context
      ): Builder[DFBool, DFBool] =
        create[DFBool, DFBool]((left, right) => (left, right))

      implicit def evDFBool_op_Const[R](implicit
          ctx: DFAny.Context,
          rConst: DFAny.Const.ToFit[Type, R]
      ): Builder[DFBool, R] =
        create[DFBool, R]((left, rightValue) => {
          val right = rConst(left.dfType, rightValue)
          (left, right)
        })

      implicit def evConst_op_DFBool[L](implicit
          ctx: DFAny.Context,
          lConst: DFAny.Const.ToFit[Type, L]
      ): Builder[L, DFBool] =
        create[L, DFBool]((leftValue, right) => {
