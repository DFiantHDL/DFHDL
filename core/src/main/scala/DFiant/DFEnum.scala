...
object DFEnum extends DFAny.Companion {
  final case class Type[E <: EnumType](enumType: E) extends DFAny.Type {
    ...
    def codeString(implicit printer: CSPrinter): String =
      s"${printer.config.TP}DFEnum(${enumType.refCodeString})"
  ...
  /**
    ...
    * }}}
    */
  def apply[E <: EnumType](
      enumType: E
  )(implicit ctx: DFAny.Context): DFAny.NewVar[Type[E]] =
    DFAny.NewVar(Type(enumType))
  def unapply(arg: DFAny.Member): Option[EnumType] =
    arg.dfType match {
  ...
  final case class Token(enumType: EnumType, value: Option[EnumType.Entry])
      extends DFAny.Token.Of[Type[EnumType], EnumType.Entry] {
    val dfType: DFAny.Type = Type(enumType)
    val width: Int         = enumType.width.getValue
    def valueToBitVector(value: EnumType.Entry): BitVector =
      value.value.toBitVector(width)
    def valueCodeString(value: EnumType.Entry)(implicit
        printer: CSPrinter
    ): String = value.codeString
  }
  object Token {
    implicit def bubbleOfToken: DFAny.Token.BubbleOfToken[Token] =
      t => Token.bubble(t.enumType)
    implicit def bubbleOfDFType[E <: EnumType]
        : DFAny.Token.BubbleOfDFType[Type[E]] = t => Token.bubble(t.enumType)
    def bubble(enumType: EnumType): Token     = Token(enumType, None)
    def apply(enumType: EnumType, entry: EnumType.Entry): Token =
      Token(enumType, Some(entry))

    type Summon[E <: EnumType, V] =
      DFAny.Token.Exact.Summon.SAM[Type[E], V, TokenE[E]]
    trait Implicits {
      implicit def __DFEnumTokenEntry[E <: EnumType, V <: E#Entry]
          : Summon[E, V] =
        (from, value) => Token(from.enumType, value).typeTag[Type[E]]
    }
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Match Pattern
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  class Pattern(set: Set[EnumType.Entry])
      extends DFAny.Pattern.OfSet[Type[EnumType], EnumType.Entry, Pattern](
        set
      ) {
    protected def matchCond(
        matchVal: DFAny.Of[Type[EnumType]],
        value: EnumType.Entry
    )(implicit
        ctx: DFAny.Context
    ): DFBool = {
  ...
  object Pattern extends PatternCO {
    ...
    object Able {
      implicit class DFEnumPattern[E <: EnumType](val right: E#Entry)
          extends Able[E#Entry]
    }
    trait Builder[LType <: DFAny.Type]
        extends DFAny.Pattern.Builder[LType, Able]
    object Builder {
      implicit def ev[E <: EnumType]: Builder[Type[E]] =
        new Builder[Type[E]] {
  ...
  object Op {
    ...
    class AbleOps[L](value: L) extends Able[L](value) {
      val left = value
      final def ===[E <: EnumType](right: DFEnum[E])(implicit
          op: `Op===`.Builder[L, DFEnum[E]]
      ) = op(left, right)
      final def =!=[E <: EnumType](right: DFEnum[E])(implicit
          op: `Op=!=`.Builder[L, DFEnum[E]]
      ) = op(left, right)
    ...
    trait Implicits {
      sealed class __DFEnumFromEntry[L <: EnumType.Entry](left: L)
          extends AbleOps[L](left)
      final implicit def __DFEnumFromEntry[L <: EnumType.Entry](
          left: L
      ): __DFEnumFromEntry[L] = new __DFEnumFromEntry(left)
      final implicit def __ofDFEnum[E <: EnumType](
          left: DFEnum[E]
      ): Able[DFEnum[E]] = new Able(left)
      final implicit class __DFEnumOps[E <: EnumType](val left: DFEnum[E]) {
        def ===[R](right: Exact[R])(implicit
            op: `Op===`.Builder[DFEnum[E], R]
        ) = op(left, right)
        def =!=[R](right: Exact[R])(implicit
            op: `Op=!=`.Builder[DFEnum[E], R]
        ) = op(left, right)
  ...
  protected abstract class OpsCompare[Op <: DFAny.Func2.Op](op: Op) {
    ...
    object Builder {
      def create[E <: EnumType, L, R](
          properLR: (L, R) => (DFEnum[E], DFEnum[E])
      )(implicit ctx: DFAny.Context): Builder[L, R] =
        (leftL, rightR) => {
      ...
      implicit def evDFEnum_op_DFEnum[E <: EnumType](implicit
          ctx: DFAny.Context
      ): Builder[DFEnum[E], DFEnum[E]] =
        create[E, DFEnum[E], DFEnum[E]]((left, right) => (left, right))

      implicit def evDFEnum_op_Entry[E <: EnumType, R <: E#Entry](implicit
          ctx: DFAny.Context,
          const: DFAny.Const.ToFit[Type[E], R]
      ): Builder[DFEnum[E], R] =
        create[E, DFEnum[E], R]((left, rightEntry) =>
          (left, const(left.dfType, rightEntry))
        )

      implicit def evEntry_op_DFEnum[E <: EnumType, L <: E#Entry](implicit
          ctx: DFAny.Context,
          const: DFAny.Const.ToFit[Type[E], L]
      ): Builder[L, DFEnum[E]] =
        create[E, L, DFEnum[E]]((leftEntry, right) =>
          (const(right.dfType, leftEntry), right)
        )
    }
  }
...
sealed abstract class EnumType(implicit meta: Meta) {
  private[DFiant] var entries: immutable.ListMap[BigInt, EnumType.Entry] =
    immutable.ListMap.empty[BigInt, EnumType.Entry]
  ...
  final def name(implicit getSet: MemberGetSet): String =
    getSet.getGlobalTag[EnumType.NameTag](this) match {
...
object EnumType {
  ...
  trait LowPriority {
    implicit def topCtx(implicit
        lp: shapeless.LowPriority,
        meta: Meta
    ): Context = new Context(None, meta)
  }
  object Context extends LowPriority {
    implicit def fromCtx(implicit ctx: DFMember.Context, meta: Meta): Context =
      new Context(Some(ctx.container), meta)
  }
  ...
  sealed trait Entry {
    ...
    final def getFullName(implicit getSet: MemberGetSet) =
      s"${enumType.name}.$name"
    def codeString(implicit printer: CSPrinter): String = getFullName
    final override def toString: String                 = name
  }
  object Entry {
    implicit def csoEnum[E <: Entry]: CodeStringOf[E] =
      new CodeStringOf[E] {
        override def apply(t: E)(implicit printer: CSPrinter): String =
          t.codeString
      }
  }
  ...
  object Encoding {
    ...
    object Default extends Encoding {
      def calcWidth(entryCount: Int): Int =
        BigInt(entryCount - 1).bitsWidth(false)
      val func: Int => BigInt = t => BigInt(t)
      def codeString(implicit printer: CSPrinter): String =
        "Enum.Encoding.Default"
    }
    ...
    case class StartAt[V <: Int with Singleton](value: V) extends Encoding {
      def calcWidth(entryCount: Int): Int =
        BigInt(entryCount - 1 + value).bitsWidth(false)
      val func: Int => BigInt = t => BigInt(t + value)
      def codeString(implicit printer: CSPrinter): String =
        s"Enum.Encoding.StartAt($value)"
    }
  ...
  abstract class Auto[E <: Encoding](val encoding: E = Encoding.Default)(
    ...
    type EntryWidth = Int
    final lazy val width: TwoFace.Int[EntryWidth] =
      encoding.calcWidth(entries.size)
    ...
    class Entry private[DFiant] (implicit
        val enumType: EnumType,
        private[DFiant] val meta: Meta
    ) extends EnumType.Entry {
  ...
  /**
    ...
    * @param meta Provides an implicit meta context. Do not manually apply.
    */
  abstract class Manual[Width <: Int with Singleton](
    ...
    private def entriesCodeString(implicit printer: CSPrinter): String = {
      ...
      entries
        .map(e =>
          f"\n  $SC final $SC val ${e._2.name} ${ALGN(0)}= $DF Entry(${cs(e._1.toBitVector(width))})"
        )
        .mkString
    }
    final def codeString(implicit printer: CSPrinter): String = {
    ...
    class Entry private[DFiant] (val value: BigInt, val enumType: EnumType)(
        implicit private[DFiant] val meta: Meta
    ) extends EnumType.Entry {
      enumType.update(this)
      latestEntryValue = Some(value)
    }

    def Entry[T <: Int with Singleton](t: T)(implicit
        check: RequireMsg[BitsWidthOf.CalcInt[T] <= Width, Msg[
          BitsWidthOf.CalcInt[T]
        ]],
        enumOwner: EnumType,
        meta: Meta
    ): Entry = new Entry(t, enumOwner)

    def Entry(t: BigInt)(implicit
        enumOwner: EnumType,
        meta: Meta
    ): Entry = {
    ...
    private type Msg2[EW] = "Entry value width (" + ToString[
      EW
    ] + ") is different than the enumeration width (" + ToString[Width] + ")"
    def Entry[W](t: DFBits.TokenW[W])(implicit
        check: RequireMsg[W == Width, Msg2[W]],
        enumOwner: EnumType,
        meta: Meta
    ): Entry = new Entry(t.valueBits.toBigInt, enumOwner)

    def Entry(t: DFBits.Token)(implicit
        enumOwner: EnumType,
        meta: Meta
    ): Entry = {
      require(
        t.width == width.getValue,
        s"`${meta.name}` entry value width (${t.width}) is different than the enumeration width ($width)"
      )
