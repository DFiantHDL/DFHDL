...
final case class ClockParams(
  name : String, edge : ClockParams.Edge = ClockParams.Edge.Rising
...
object ClockParams {
  ...
  def get(implicit getSet: MemberGetSet) : ClockParams =
    getSet.designDB.top.getTagOf[ClockParams].getOrElse(default)
}
