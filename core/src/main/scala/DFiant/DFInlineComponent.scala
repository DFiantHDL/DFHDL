...
abstract class DFInlineComponent[Type <: DFAny.Type](dfType: Type)(implicit
    ctx: ContextOf[DFInlineComponent[_]]
) extends DFDesign
    with DFAny.Of[Type] {
  val rep: DFInlineComponent.Rep
  override private[DFiant] lazy val inlinedRep: Option[DFInlineComponent.Rep] =
    Some(rep)
...
object DFInlineComponent {
  trait Rep extends Product with Serializable {
    protected[DFiant] def =~(that: DFInlineComponent.Rep)(implicit
        getSet: MemberGetSet
    ): Boolean
  ...
  object Block {
    def unapply(arg: DFDesign.Block): Option[Rep] =
      arg match {
...
object EdgeDetect {
  ...
  final case class Rep(bitRef: DFInlineComponent.Ref, edge: EdgeDetect.Edge)
      extends DFInlineComponent.Rep {
    protected[DFiant] def =~(
        that: DFInlineComponent.Rep
  ...
  object Rep {
    def apply(bit: DFAny.Member, edge: Edge)(implicit ctx: DFNet.Context): Rep =
      new Rep(bit, edge)
    object Unref {
      def unapply(
          arg: Rep
      )(implicit getSet: MemberGetSet): Option[(DFAny.Member, Edge)] =
        arg match {
