...
abstract class TagsOf[-D <: DFDesign] extends DisallowExternalExtensions {
  ...
  final def getTagMap =
    taggedMembers.foldRight(Map.empty[DFMember, DFMember.CustomTagMap]) {
      case (tm, tagMap) =>
        tagMap.get(tm.member) match {
          case Some(memberTags) => tagMap + (tm.member -> (memberTags + tm.tag))
          case None             => tagMap + (tm.member -> Map(tm.tag))
        }
    }

  private[DFiant] def tagMember[M <: DFMember, CT <: DFMember.CustomTagOf[
    M
  ]: ClassTag](
      member: => M,
      tag: CT
  ...
  implicit class __DesignExt[D2 <: DFDesign](design: => D2) {
    def !!![CT <: DFMember.CustomTagOf[DFDesign.Block]: ClassTag](
        tag: CT
    ): TaggedMember[DFDesign.Block] =
  ...
  def !![D2 <: D](moreTags: TagsOf[D2]): TagsOf[D] =
    new TagsOf[D] {
      moreTags.attachDesign(dsn.asInstanceOf[D2])
      taggedMembers = this.taggedMembers ++ moreTags.taggedMembers
    }
}
protected class TaggedMember[+M <: DFMember](
    container: TagsOf[_],
    _member: => M,
    val tag: (ClassTag[_], DFMember.CustomTag)
) {
  def !![CT <: DFMember.CustomTagOf[M]: ClassTag](tag: CT): TaggedMember[M] =
    container.tagMember(_member, tag)
