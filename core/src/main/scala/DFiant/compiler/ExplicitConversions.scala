...
final class ExplicitConversions[D <: DFDesign](c: IRCompilation[D]) {
  ...
  def carryMathConversion: IRCompilation[D] = {
    val patchList = designDB.members.flatMap {
      case func @ DFAny.Func2.Unref(
            dfType,
            leftArg,
            op: DFAny.Func2.Op.Carry,
            rightArg,
            _,
            _
          ) =>
        val dsn = new MetaDesign() {
          ...
          private val tokenFuncNC
              : (_ <: DFAny.Token, _ <: DFAny.Token) => DFAny.Token =
            (op, dfType) match {
              case (Op.+^, DFUInt.Type(_)) =>
                (l: DFUInt.Token, r: DFUInt.Token) => l + r
              case (Op.-^, DFUInt.Type(_)) =>
                (l: DFUInt.Token, r: DFUInt.Token) => l - r
              case (Op.*^, DFUInt.Type(_)) =>
                (l: DFUInt.Token, r: DFUInt.Token) => l * r
              case (Op.+^, DFSInt.Type(_)) =>
                (l: DFSInt.Token, r: DFSInt.Token) => l + r
              case (Op.-^, DFSInt.Type(_)) =>
                (l: DFSInt.Token, r: DFSInt.Token) => l - r
              case (Op.*^, DFSInt.Type(_)) =>
                (l: DFSInt.Token, r: DFSInt.Token) => l * r
          ...
          DFAny.Func2.forced(dfType, left, opNC, right)(tokenFuncNC) setTags (
            _ => func.tags
          )
        }
        Some(
  ...
  def explicitConversions: IRCompilation[D] = {
    val patchList = designDB.members.flatMap {
        ...
        if (conv) {
          val dsn = new MetaDesign() {
            val updatedFromVal = (toVal, fromVal) match {
              case (DFUInt(toWidth), DFUInt(fromWidth))
                ...
                fromVal
                  ...
                  .member
                  .anonymize
              case (DFSInt(toWidth), DFSInt(fromWidth))
                  if toWidth > fromWidth =>
                fromVal
                  .asValOf[DFSInt.Type[Int]]
                  .resize(toWidth)
      ...
      //assert on bit
      case asrt @ sim.DFSimMember.Assert.Unref(
            Some(cond @ DFBit()),
            msg,
            severity,
            _,
        ...
        val dsn = new MetaDesign() {
          sim.DFSimMember.Assert(
            Some(toggleLogical(cond).asInstanceOf[DFBool]),
            msg,
            severity
          )
        }
