...
package object compiler {
  implicit def SanityCheck[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): SanityCheck[D] = new SanityCheck[D](c)
  implicit def FixAnonymous[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): FixAnonymous[D] = new FixAnonymous[D](c)
  implicit def evOrderMembers[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): OrderMembers[D] = new OrderMembers[D](c)
  implicit def ForceOthersCaseCoverage[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ForceOthersCaseCoverage[D] = new ForceOthersCaseCoverage[D](c)
  implicit def UniqueDesigns[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): UniqueDesigns[D] = new UniqueDesigns[D](c)
  implicit def NamedSelection[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): NamedAliases[D] = new NamedAliases[D](c)
  implicit def UniqueNames[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): UniqueNames[D] = new UniqueNames[D](c)
  implicit def FlattenNames[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): FlattenNames[D] = new FlattenNames[D](c)
  implicit def ConvertMatchToIf[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ConvertMatchToIf[D] = new ConvertMatchToIf[D](c)
  implicit def ExplicitPrev[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ExplicitPrev[D] = new ExplicitPrev[D](c)
  implicit def ExplicitConversions[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ExplicitConversions[D] = new ExplicitConversions[D](c)
  implicit def ExplicitNamedVars[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ExplicitNamedVars[D] = new ExplicitNamedVars[D](c)
  implicit def ViaPortConnection[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ViaPortConnection[D] = new ViaPortConnection[D](c)
  implicit def MoveCBDesigns[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): MoveCBDesigns[D] = new MoveCBDesigns[D](c)
  implicit def ControlDesigns[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): ControlDesigns[D] = new ControlDesigns[D](c)
  implicit def Flatten[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): Flatten[D] = new Flatten[D](c)
  implicit def Calculator[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): InitCalc[D] = new InitCalc[D](c)
  implicit def PathEvaluator[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): PathEvaluator[D] = new PathEvaluator[D](c)
  implicit def Pipelining[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): Pipelining[D] = new Pipelining[D](c)
  implicit def SingleStepPrev[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): SingleStepPrev[D] = new SingleStepPrev[D](c)
  implicit def RTL[D <: DFDesign, C](c: C)(implicit
      conv: C => IRCompilation[D]
  ): RTL[D] = new RTL[D](c)
  ...
  trait PostCompiler[D <: DFDesign, B <: BackendStage] {
    def apply(
        fromStage: BackendStage.Compilation[D, B]
    ): BackendStage.Compilation[D, B]
  }
  object PostCompiler {
    implicit def defaultDoesNothing[D <: DFDesign, B <: BackendStage]
        : PostCompiler[D, B] =
  ...
  final implicit class SourceElementOps(sourceElement: SourceElement) {
    ...
    def getDependencies(dependencyKind: DependencyKind)(implicit
        ...
        case _ =>
          ...
          sourceElement.srcVal match {
                ...
                case DependencyKind.Data =>
                  version match {
                    ...
                    case SourceVersion.IfElse(
                      ...
                      sourceElement.versioned(
                        dcl,
                        fallbackVersion
                      ) :: branchVersions.flatMap {
                        case (condSrc, version) =>
                          List(
                            condSrc.elements.head,
                    ...
                    case SourceVersion.Match(
                      ...
                      val caseElements = caseVersions.map {
                        case (_, version) =>
                          sourceElement.versioned(dcl, version)
                      }
                      fallbackVersionOption
            ...
            case SourceValue.Func2(func) =>
              List(func.leftArgRef.getSource, func.rightArgRef.getSource)
                .flatMap(_.elements)
  ...
  final implicit class BlockVersionsOps(
      blockVersions: DependencyContext.BlockVersions
  ) {
    @tailrec def getLatestVersionIn(
      ...
      blockVersions.get(currentBlock) match {
        case Some(versions) =>
          SourceVersion.Idx(currentBlock, versions.length - 1)
        case None =>
          currentBlock match {
  ...
  implicit class PendingTableOps(pendingTable: PendingTable) {
    def waitsFor(sourceElement: SourceElement): Boolean =
      pendingTable.get(sourceElement.srcVal) match {
        case Some(bitSet) =>
          val elementBitSet = BitSet.fromSpecific(
            sourceElement.relBitLow to sourceElement.relBitHigh
          )
    ...
    def addPending(sourceElement: SourceElement): PendingTable = {
      val elementBitSet =
        BitSet.fromSpecific(sourceElement.relBitLow to sourceElement.relBitHigh)
      pendingTable.get(sourceElement.srcVal) match {
        case Some(bitSet) =>
          pendingTable.updated(sourceElement.srcVal, bitSet | elementBitSet)
        case None => pendingTable + (sourceElement.srcVal -> elementBitSet)
      }
    }
  }
  type EvaluationMap[T] = Map[SourceValue, T]
  implicit class EvaluationMapOps[T](evaluationMap: EvaluationMap[T])(implicit
      elementEvaluator: ElementEvaluator[T]
  ) {
    def evaluate(sourceElement: SourceElement): T =
      elementEvaluator.applyElement(
        evaluationMap(sourceElement.srcVal),
        sourceElement
      )
  }
}
