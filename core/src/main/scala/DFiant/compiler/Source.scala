...
final case class SourceElement(
  ...
  val dfType: DFAny.Type =
    if (relWidth == srcVal.width) srcVal.dfType else DFBits.Type(relWidth)
  ...
  def bits(relBitHigh: Int, relBitLow: Int): List[SourceElement] =
    if (relBitHigh < relBitLow) Nil
      ...
      List(
        copy(
          relBitHigh = this.relBitLow + relBitHigh,
          relBitLow = this.relBitLow + relBitLow
        )
      )
  def concat(elements: List[SourceElement]): List[SourceElement] =
    elements match {
      case ::(right, rightNext) =>
        ...
        val coalesceNormal =
          ((!left.reversed || left.relWidth == 1) && (!right.reversed || right.relWidth == 1))
        val coalesceReverse =
          ((left.reversed || left.relWidth == 1) && (right.reversed || right.relWidth == 1))

        if (
          matchingArguments && coalesceNormal && left.relBitLow == right.relBitHigh + 1
        )
          SourceElement(
            srcVal,
            withInit,
          ...
          matchingArguments && coalesceReverse && left.relBitHigh == right.relBitLow - 1
        )
          SourceElement(
            srcVal,
            withInit,
          ...
          left :: elements //cannot be coalesced, so this element is just concatenated to the rest
      case Nil => List(left)
    }
  def propertiesFrom(element: SourceElement): SourceElement =
    copy(
      reversed = reversed ^ element.reversed,
      inverted = inverted ^ element.inverted,
...
sealed abstract class SourceValue(val dfType: DFAny.Type)
    extends Product
    with Serializable {
  val width: Int = dfType.width
  def codeString(implicit
...
object SourceValue {
  ...
  final case class Dcl(dcl: DFAny.Dcl, version: SourceVersion)
      extends SourceValue(dcl.dfType) {
    def codeString(implicit
        dependencyContext: DependencyContext,
        printer: CSPrinter
    ): String = {
      ...
      version match {
        ...
        case SourceVersion.IfElse(_, branchVersions, fallbackVersion) =>
          val branchVersionStr = branchVersions.map(bv =>
            (bv._1.codeString, SourceValue.Dcl(dcl, bv._2).codeString)
          )
        ...
        case SourceVersion.Match(_, caseVersions, fallbackVersion) =>
          val caseVersionStr = caseVersions.map(bv =>
            (bv._1.codeString, SourceValue.Dcl(dcl, bv._2).codeString)
          )
          val fallBackStr =
            fallbackVersion.map(fb => SourceValue.Dcl(dcl, fb).codeString)
  ...
  final case class Func2(dfVal: DFAny.Func2) extends SourceValue(dfVal.dfType) {
    def codeString(implicit
      ...
      val leftStr = dependencyContext
        .dependencyMap(dfVal.leftArgRef)
        .codeString
        .applyBrackets()
      val rightStr = dependencyContext
        .dependencyMap(dfVal.rightArgRef)
        .codeString
        .applyBrackets()
  ...
  final case class ApplySel(
      override val dfType: DFAny.Type,
      relSrc: Source,
      idxSrc: Source
  ) extends SourceValue(dfType) {
    def codeString(implicit
        dependencyContext: DependencyContext,
        printer: CSPrinter
    ): String = {
...
final case class Source(dfType: DFAny.Type, elements: List[SourceElement]) {
  lhs =>
  ...
  def bitsWL(relWidth: Int, relBitLow: Int): Source =
    bits(relBitLow + relWidth - 1, relBitLow)
    ...
    if (dfType.width < this.width) bits(dfType.width - 1, 0).as(dfType)
      ...
      dfType match {
        case DFSInt.Type(_) =>
          Source(
            dfType,
            List.fill(deltaWidth)(
              bits(width - 1, width - 1).elements.head
            ) ++ elements
          )
        case DFUInt.Type(_) | DFBits.Type(_) =>
          Source(
            dfType,
            SourceElement(
              SourceValue.Const(DFBits.Token(deltaWidth, 0)),
              withInit = true
            ) concat elements
          )
      }
  ...
  def codeString(implicit dependencyContext: DependencyContext): String = {
    ...
    elements match {
      case SourceElement(srcVal, _, _, _, false, false, _, _) :: Nil
          if srcVal.dfType == dfType =>
        concatStr
      case _ =>
        dfType match {
...
object Source {
  def apply(dfType: DFAny.Type, element: SourceElement): Source =
    Source(dfType, element :: Nil)
  def apply(
      dcl: DFAny.Dcl,
      sourceVersion: SourceVersion,
      withInit: Boolean
  ...
  object Const {
    def apply(token: DFAny.Token): Source =
      Source(
        token.dfType,
        SourceElement(SourceValue.Const(token), withInit = true)
      )
  }
  object Idx {
  ...
  object Latest {
    def apply(dfVal: DFAny.Member): Source =
      dfVal match {
        case dcl: DFAny.Dcl =>
          Source(dcl, SourceVersion.Latest, withInit = true)
        case func2: DFAny.Func2 =>
          Source(
            dfVal.dfType,
            SourceElement(SourceValue.Func2(func2), withInit = true)
          )
      }
  }
  object ApplySel {
    def apply(dfType: DFAny.Type, relSrc: Source, idxSrc: Source): Source =
      Source(
        dfType,
        SourceElement(
          SourceValue.ApplySel(dfType, relSrc, idxSrc),
          withInit = false
        )
      )
  }
