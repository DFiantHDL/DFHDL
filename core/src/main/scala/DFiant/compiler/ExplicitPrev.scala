...
final class ExplicitPrev[D <: DFDesign](c: IRCompilation[D]) {
      ...
      case DFAny.Port.Out() | DFAny.Var() =>
        designDB.getConnectionTo(value) match {
          ...
          case None =>
            ...
            if (scope.isConsumingPrevAt(access)) currentSet union Set(value)
            else currentSet
        }
      case _ => currentSet
    }
  ...
  final implicit class ScopeMap(sm: Map[DFAny.Member, AssignedScope]) {
    def assignTo(
        toVal: DFAny.Member,
        assignBitSet: immutable.BitSet
    ): Map[DFAny.Member, AssignedScope] =
      sm + (toVal -> sm
        .getOrElse(toVal, AssignedScope.empty)
        .assign(assignBitSet))
  ...
  @tailrec private def getImplicitPrevVars(
      ...
      scopeMap: Map[DFAny.Member, AssignedScope],
      currentSet: Set[DFAny.Member]
      ...
      case (nextBlock: DFBlock) :: rs
        ...
        val (updatedSet, updatedScopeMap): (
          ...
          case DFConditional.IfElseBlock(Some(condRef), _, _, _) =>
            (
              consumeFrom(condRef.get, scopeMap, currentSet),
              scopeMap.branchEntry(firstBranch = true)
            )
        ...
        val (updatedSet, updatedScopeMap) : (Set[DFAny.Member], Map[DFAny.Member, AssignedScope]) = r match {
          ...
          case assert: DFSimMember.Assert =>
            val dfAnySet: Seq[DFAny.Member] =
              (assert.msgRef.seq.collect {
                case Left(x) => x
              } ++ assert.condOptionRef).map(_.get)
          ...
          case matchBlock: DFConditional.MatchHeader =>
            (
              consumeFrom(matchBlock.matchValRef.get, scopeMap, currentSet),
              scopeMap
            )
        ...
        val updatedSet = currentBlock match {
          case d: DFDesign.Block if remaining.isEmpty =>
            val outPorts: List[DFAny.Member] =
              designDB.designMemberTable(d).collect {
                case p @ DFAny.Port.Out() => p
                case p @ DFAny.NewVar()   => p
              }
            outPorts.foldLeft(currentSet) {
              case (cs, p) => consumeFrom(p, scopeMap, cs)
            }
          case _ =>
            currentSet
        }
  ...
  def explicitPrev: IRCompilation[D] = {
    ...
    val (explicitPrevSet, defaultsSet) =
      currentSet.partition(p => scopeMap(p).hasAssignments)
    val defaultsPatchList =
      Nil //Used to contain a default set of initialization. Currently, there is no real need.
//      defaultsSet
//      .flatMap {
...
//        case _ => None
//      }
    val outs = explicitPrevSet
      .collect { case p @ DFAny.Port.Out() => p }
      .toList
      .groupBy(p => p.getOwnerDesign)
      .toList
      ...
      case (block, portSet) =>
        val portDsns = portSet.map { p =>
          val dsn = new MetaDesign() {
            ...
            private val p_sig_noinit =
              DFAny.NewVar(p.dfType) setName (s"${p.name}_sig")
            final val p_sig = __getset.set[DFAny.Member](p_sig_noinit)(_ =>
              p_sig_noinit.member
                .asInstanceOf[DFAny.Dcl]
                .copy(externalInit = p.externalInit)
            )
            DFNet.Assignment(
              p_var,
              DFAny.Alias.Prev(p_sig, 1, DFAny.Alias.Prev.State)
            )
          }
          (p, dsn)
        }
          ...
          case (p, dsn) =>
            p -> Patch.Add(
              ...
              Patch.Add.Config.ReplaceWithFirst(
                ...
                Patch.Replace.RefFilter.Inside(block)
              )
            )
        }
        val addedAssignments = block -> Patch.Add(
          new MetaDesign() {
            portDsns.foreach {
              case (p, dsn) =>
      ...
      case e =>
        Some(
          e -> Patch.Add(
            new MetaDesign() {
              DFNet
                .Assignment(e, DFAny.Alias.Prev(e, 1, DFAny.Alias.Prev.State))
            },
            Patch.Add.Config.After
          )
        )
