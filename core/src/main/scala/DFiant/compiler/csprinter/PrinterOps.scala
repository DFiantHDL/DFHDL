...
final class PrinterOps[D <: DFDesign, C](c : C)(implicit conv : C => Compilation[D]) {
  ...
  private def blockBodyCodeString(owner : DFOwner, members : List[DFMember], lateConstruction : Boolean)(
    ...
    val membersCodeString = members.flatMap {
      ...
      case cb : DFConditional.Block => Some(cb.codeString(blockBodyCodeString(cb, fixedDB.blockMemberTable(cb), lateConstruction)))
      ...
      case a : DFAny.Member if !a.isAnonymous =>
        val customTagInfo = showTagsFilter match {
          case Some(tagsFilter) =>
            ...
            if (visibleTags.nonEmpty) visibleTags.mkString(s" ${DF}tag ", s" ${DF}tag ", "")
            else ""
          case None => ""
        }
