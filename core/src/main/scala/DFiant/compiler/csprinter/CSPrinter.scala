...
object CSPrinter {
  ...
  object Config {
    ...
    case object ShowInits extends Config {
      override val showTagsFilter: Option[DFMember.CustomTag => Boolean] =
        Some({
  ...
  implicit def defaultPrinter(implicit __getSet: MemberGetSet): CSPrinter =
    new CSPrinter {
      val getSet: MemberGetSet     = __getSet
      val config: CSPrinter.Config = CSPrinter.Config.Default
    }
}
