...
final class SingleStepPrev[D <: DFDesign](c : IRCompilation[D]) {
  ...
  @tailrec private def getRelValStep(relValRef : DFAny.Alias.RelValRef, step : Int, kind : Kind) : (DFAny.Member, Int) = {
    val relVal = relValRef.get
    relVal match {
      case DFAny.Alias.Prev(_, nextRelValRef, nextStep, nextKind, _, _) if nextKind == kind =>
          ...
          if (step > prevList.length) {
            val dsn = new MetaDesign() {
              val (_, updatedPrevList) =
                (prevList.length + 1 to step).foldLeft((prevList.headOption.getOrElse(relVal), prevList)) {
                  case ((rv, list), s) =>
                    val prevName = if ((step == s) & !p.isAnonymous) p.name else rv.name match {
                      case prevNamePattern(pre, num) => s"$pre${num.toInt + 1}"
                      case sigNamePattern(pre) => s"${pre}_${_kind.op}${s - prevList.length}"
                      case _ => s"${rv.name}_${_kind.op}${s - prevList.length}"
                    }
