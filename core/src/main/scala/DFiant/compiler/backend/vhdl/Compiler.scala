...
final class Compiler[D <: DFDesign](c: IRCompilation[D]) {
    ...
    val (_, statements) = printer.getSet.designDB
      ...
      .foldRight(("", List.empty[String])) {
        ...
        case (
          ...
          (
            Case.When(Case.Choice.Others(), getProcessStatements(cb)),
            statements
          )
        case (
              cb @ DFConditional.CaseBlock(_, _, Some(pattern), _, _),
              (whens, statements)
            ) =>
          val when =
            Case.When(Case.Choice.Pattern(pattern), getProcessStatements(cb))
  ...
  private def matchToIfs(getSet: MemberGetSet)(implicit
      revision: Revision
        ...
        matchHeaders.filter { mh =>
          mh.matchConfig match {
            case MatchConfig.NoOverlappingCases =>
              getSet.designDB.caseWithDontCare(mh)
    ...
    matchHeaders.filter { mh =>
      mh.matchValRef.get match {
        ...
        case v @ DFBits(width) =>
          ...
          if (
            !hasCase_ && ((BigInt.maxUnsignedFromWidth(
              width
            ) + 1) == cases.size)
          ) true
          else false
  ...
  def vhdlCompile[R <: Revision](implicit
      revision: R
...
//       .controlDesigns
    ...
    val files = designDB.designMemberList.flatMap {
      ...
      case (design: DFDesign.Block, members)
        ...
        val (ports, signals, variables) = members.foldRight(
          (List.empty[String], List.empty[String], List.empty[String])
        ) {
          case (p @ DFAny.Port.In(), (ports, signals, variables)) =>
            (
              Port(p.name, Port.Dir.In(), Type(p.dfType), Init(p)) :: ports,
              signals,
              variables
            )
          case (p @ DFAny.Port.Out(), (ports, signals, variables)) =>
            (
              Port(p.name, Port.Dir.Out(), Type(p.dfType), Init(p)) :: ports,
              signals,
              variables
            )
          case (s: DFAny.Member, (ports, signals, variables))
              if !s.isAnonymous && (designDB.getConnectionTo(s).isDefined ||
                s.tags.customTags.values.exists {
                  case _: RTL.Tag[_] => true
                  case _             => false
                } ||
                designDB
                  .getAssignmentsFrom(s)
                  .exists(x => x.isTaggedWith(RTL.Tag.Mod.Reg))) =>
            (
              ports,
              Signal(s.name, Type(s.dfType), Init(s)) :: signals,
              variables
            )
          case (v: DFAny.Member, (ports, signals, variables))
              if !v.isAnonymous =>
            (
              ports,
              signals,
              Variable(v.name, Type(v.dfType), Init(v)) :: variables
            )
          case (_, psv) => psv
        }
    ...
    val packageFile =
      BackendStage.File(s"${PackageFile.Name()}.vhdl", PackageFile())
    BackendStage
      .Compilation[D, Backend[R]](c.dsn, designDB, packageFile :: files)
  }
}
