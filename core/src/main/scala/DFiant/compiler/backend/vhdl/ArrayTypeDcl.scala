...
private object ArrayTypeDcl {
  def arrayTypeName(dfType: DFVector.Type[_ <: DFAny.Type,_])(implicit printer : Printer) : String = {
    s"${Type.typeName(dfType.cellType)}_arr${dfType.cellNum}"
  }
  def apply(dfType: DFVector.Type[_ <: DFAny.Type,_])(implicit printer : Printer) : String = {
