...
private object EnumTypeDcl {
  def constants(enumType: EnumType)(implicit printer: Printer): String = {
    ...
    enumType.entries.toList
      .sortBy(x => x._1)
      .map { x =>
        s"$KW constant ${enumEntryFullName(x._2)} ${ALGN(0)}: ${enumTypeName(
          enumType
        )} := ${uintRep(enumType, x._1)};"
      }
      .mkString("\n")
  }
  ...
  def enumTypeName(enumType: EnumType)(implicit printer: Printer): String =
    s"E_${enumType.name}"
  def enumEntryFullName(
      entry: EnumType.Entry
  )(implicit printer: Printer): String =
    s"${enumTypeName(entry.enumType)}_${entry.name}"
  object DefaultEncoding {
    def unapply(enumType: EnumType): Boolean =
      enumType match {
        case auto: EnumType.Auto[_]
            if auto.encoding == EnumType.Encoding.Default =>
          true
        case _ => false
      }
  }
    ...
    enumType match {
      ...
      case _ =>
        val caseItems: List[String] = enumType.entries.map {
          case (n, e) =>
            Case.When(
              s"$LIT ${uintRep(enumType, n)}",
              List(s"""$KW return "${enumEntryFullName(e)}";""")
            )
        }.toList :+ Case.When(
          Case.Choice.Others(),
          List(s"""$KW return "XXXX";""")
        )
  ...
  def apply(enumType: EnumType)(implicit printer: Printer): String = {
    ...
    enumType match {
      case DefaultEncoding() =>
        val typeList = enumType.entries.toList
          .sortBy(x => x._1)
          .map(x => EnumTypeDcl.enumEntryFullName(x._2))
      ...
      case _ =>
        ...
        val subtypeDcl =
          s"$KW subtype ${enumTypeName(enumType)} $KW is $uintType;"
        s"""$subtypeDcl
           |${constants(enumType)}
