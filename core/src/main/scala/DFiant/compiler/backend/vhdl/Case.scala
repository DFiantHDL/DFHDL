...
private object Case {
  def apply(expression: String, whens: String, allowDontCare: Boolean)(implicit
      printer: Printer
  ): String = {
  ...
  object When {
    def apply(choice: String, statements: List[String])(implicit
        printer: Printer
    ): String = {
      import printer.config._
      s"""$KW when $choice =>
  ...
  object Choice {
    object Others {
      def apply()(implicit printer: Printer): String =
        s"${printer.config.KW}others"
    }
    object Pattern {
      private def intervalBigIntToString(
          t: Interval[BigInt]
