...
private object Value {
  def const(token: DFAny.Token)(implicit printer: Printer): String = {
    ...
    token match {
      case t @ DFBits.Token(valueBits, _) if !t.isBubble =>
        revision match {
          case _ if t.width % 4 == 0 => s"""x"${valueBits.toHex}""""
          case Revision.V2008 if t.width > 3 =>
            s"""${t.width}x"${valueBits.toHexProper}""""
      ...
      case t @ DFBits.Token(_, _) => //bits token with bubbles
        ...
        lazy val hexRepOption: Option[String] =
          t.toHexString('-', allowBinMode = false)
        lazy val binRep  = t.toBinString('-')
        lazy val vhdlBin = s""""$binRep""""
        revision match {
          case Revision.V2008 if t.width > 3 =>
            hexRepOption match {
              case Some(value) if t.width % 4 == 0 || value.head != '-' =>
                s"""${t.width}x"$value""""
              case _ => vhdlBin
            }
          case _ => vhdlBin
  ...
  def func2(member: DFAny.Func2)(implicit printer: Printer): String = {
    ...
    val rightArgStr = (member.op, rightArg) match {
      case (_, DFAny.Const(_, DFUInt.Token(_, Some(value)), _, _)) =>
        s"$LIT$value"
      case (_, DFAny.Const(_, DFSInt.Token(_, Some(value)), _, _)) =>
        s"$LIT$value"
    ...
    if (dontCareComparison) {
      val comparison = revision match {
        ...
        case Revision.V2008 =>
          s"${leftArgStr.applyBrackets()} $OP?= ${rightArgStr.applyBrackets()}"
      }
      member.op match {
        case Op.== => comparison
      ...
      (leftArg, member.op, revision) match {
        case (DFBits(_), Op.<<, Revision.V93) =>
          s"$FN to_slv($FN shift_left($TP unsigned($leftArgStr), $rightArgStr))"
        case (DFBits(_), Op.>>, Revision.V93) =>
          s"$FN to_slv($FN shift_right($TP unsigned($leftArgStr), $rightArgStr))"
        case (DFUInt(_) | DFSInt(_), Op.<<, Revision.V93) =>
          s"$FN shift_left($leftArgStr, $rightArgStr)"
        case (DFUInt(_) | DFSInt(_), Op.>>, Revision.V93) =>
          s"$FN shift_right($leftArgStr, $rightArgStr)"
        case (_, Op.*, _) =>
          s"$FN resize(${leftArgStr.applyBrackets()} $OP$opStr ${rightArgStr
            .applyBrackets()}, ${leftArg.width max rightArg.width})"
        case _ =>
  ...
  def alias(member: DFAny.Alias)(implicit printer: Printer): String = {
    ...
    member match {
      ...
      case DFAny.Alias.BitsWL(dfType, _, _, relWidth, relBitLow, _, _) =>
        ...
        dfType match {
          case DFBool.Type(false) =>
            s"${bitsConv.applyBrackets()}($LIT$relBitLow)"
          case DFBits.Type(_) =>
  ...
  def ref(member: DFAny.Member)(implicit printer: Printer): String = {
    ...
    member match {
      ...
      case d: DFAny.Dcl =>
        d.getOwnerBlock match {
          case DFDesign.Block.Internal(_, _, _, Some(rep)) =>
            rep match {
              case EdgeDetect.Rep(bitRef, EdgeDetect.Edge.Rising) =>
                s"$OP rising_edge(${ref(bitRef)})"
              case EdgeDetect.Rep(bitRef, EdgeDetect.Edge.Falling) =>
                s"$OP falling_edge(${ref(bitRef)})"
