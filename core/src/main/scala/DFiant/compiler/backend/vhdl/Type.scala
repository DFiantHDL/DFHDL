...
private object Type {
  def apply(dfType: DFAny.Type)(implicit printer: Printer): String = {
    ...
    dfType match {
      case DFBits.Type(width) =>
        s"$TP std_logic_vector($LIT${width - 1} $KW downto $LIT 0)"
      case DFUInt.Type(width) =>
        s"$TP unsigned($LIT${width - 1} $KW downto $LIT 0)"
      case DFSInt.Type(width) =>
        s"$TP signed($LIT${width - 1} $KW downto $LIT 0)"
