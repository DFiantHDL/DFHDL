...
object BackendStage {
  final case class Compilation[D <: DFDesign, B <: BackendStage](
    ...
    override def toString: String =
      s"The Design ${db.top.designType} is compiled. The files are (not committed):\n ${fileSeq
        .map(f => f.name)
        .mkString(", ")}"
  }
  ...
  final case class CommittedCompilation[D <: DFDesign, B <: BackendStage](
    ...
    def addExternalFiles(
        fileNameSeq: Seq[String]
    ): CommittedCompilation[D, B] = {
      fileNameSeq.foreach(f =>
        require(
          new java.io.File(f).exists(),
          s"Added external file $f doesn't exist"
        )
      )
    ...
    def addExternalFiles(
        fileName: String*
    )(implicit di: DummyImplicit): CommittedCompilation[D, B] =
    ...
    override def toString: String =
      s"Design ${db.top.designType} committed as the following files:\n ${fileNameSeq.mkString("\n")}"
  }

  @implicitNotFound(
    "Missing a compiler import (e.g., `import compiler.backend.vhdl.v2008`)"
  )
  trait Compiler[B <: BackendStage] {
    def apply[D <: DFDesign](
        c: IRCompilation[D]
