...
object RTL {
  ...
  abstract class ClkRstDesign(clkParams : ClockParams, rstParams : ResetParams, simulation : Boolean)(
    ...
    final lazy val clk = {
      _hasClk = true
      if (simulation) DFBit().forcedInit(Seq(DFBool.Token(clkInit))).setName(clkParams.name) tag RTL.Tag.Clk
      else DFBit() <> IN tag RTL.Tag.Clk setName(clkParams.name)
    }
    final lazy val rst = {
      _hasRst = true
      if (simulation) DFBit().forcedInit(Seq(DFBool.Token(rstInit))).setName(rstParams.name) tag RTL.Tag.Rst
