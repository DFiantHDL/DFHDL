...
final class RTL[D <: DFDesign](c: IRCompilation[D]) {
  private val designDB =
    c.singleStepPrev.initCalc.explicitNamedVars.db
  ...
  private def getClockedDB
      : (DFDesign.DB, Map[DFDesign.Block, RTL.ClkRstDesign]) = {
    import designDB.__getset
    val addedClkRst: mutable.Map[DFDesign.Block, RTL.ClkRstDesign] =
      mutable.Map()
    val patchList = designDB.designMemberList.flatMap {
      case (block, members) =>
        ...
        lazy val hasPrevClk = members.exists {
          ...
          case _: DFSimMember =>
            true //simulation members are dependent on clock
          case _ => false
        }
        lazy val hasPrevRst = members.exists {
          case p: DFAny.Alias.Prev =>
            p.getInit match {
              case Some(_ :+ _) => true
              case _            => false
            }
          case _: DFSimMember =>
            true //simulation members are dependent on reset
          case _ => false
        }
        ...
        if (hasBlockClk || hasBlockRst || hasPrevClk || hasPrevRst) {
          val dsnClkRst =
            new RTL.ClkRstDesign(clockParams, resetParams, topSimulation) {
  ...
  def toRTLForm: IRCompilation[D] = {
    final case class PrevReplacements(
        prevRegDcl: DFAny.Dcl,
        relVal: DFAny.Member,
        prevPatch: List[(DFMember, Patch)]
    ) {
      private var sig: DFAny.Member = _
      def sigAssign(implicit ctx: DFBlock.Context): Unit =
        sig = (relVal, prevRegDcl) match {
          case (DFAny.In(), _) => relVal
          case _ if !relVal.name.endsWith("_sig") =>
            implicit val __getSet: MemberGetSet = ctx.db.getSet
            val sig =
              DFAny.NewVar(prevRegDcl.dfType) setName s"${relVal.name}_sig"
            sig.assign(relVal)
            sig
          case _ => relVal
        }
      def rstAssign(implicit ctx: DFBlock.Context): Unit =
        relVal.getInit match {
          case Some(i +: _) =>
            val initConst = DFAny.Const.forced(i)
            prevRegDcl.assign(initConst)
          case _ =>
        }
      def clkAssign(implicit ctx: DFBlock.Context): Unit =
        prevRegDcl.assign(sig)
    }
    val (clockedDB, addedClkRst) = getClockedDB
    val patchList = clockedDB.designMemberList.flatMap {
      case (block, members) =>
        ...
        val prevReplacements: List[PrevReplacements] = members.collect {
          case prevNet @ DFNet.Assignment(
                xyz_prev @ DFAny.NewVar(),
                xyzDOTprev @ DFAny.Alias.Prev.Unref(_, relVal, _, _, _, _),
                _,
                _
              ) =>
            ...
            val prevReg =
              xyz_prev.copy(externalInit = prevVarInit) tag RTL.Tag.Mod.Reg
            val prevPatch =
              Assigned(designDB.memberTable(xyz_prev)).size match {
                case 1 => //remove the net
                  List(
                    prevNet -> Patch.Remove,
                    xyz_prev -> Patch
                      .Replace(prevReg, Patch.Replace.Config.FullReplacement),
                    xyzDOTprev -> Patch
                      .Replace(prevReg, Patch.Replace.Config.ChangeRefAndRemove)
                  )
                case _ =>
                  val newVar =
                    xyz_prev.setNameSuffix("_var").copy(externalInit = None)
                  List(
                    xyz_prev -> Patch.Add(
                      List(prevReg, newVar),
        ...
        if (prevReplacements.nonEmpty) {
          ...
          val prevDsn =
            new RTL.ClkRstDesign(clockParams, resetParams, topSimulation) {
              ...
              private def rstBlock(): Unit =
                prevReplacements.foreach(pr => pr.rstAssign)
              private def clkBlock(): Unit =
                prevReplacements.foreach(pr => pr.clkAssign)
              ...
              if (hasPrevRst) {
                ...
                resetParams.mode match {
                  case Mode.Async =>
                    ifdf(rstCond)(rstBlock()).elseifdf(clkCond)(clkBlock())
                  case Mode.Sync =>
                    ifdf(clkCond)(ifdf(rstCond)(rstBlock()).elsedf(clkBlock()))
                }
              } else
          ...
          val clkPatch = (prevDsn.clk.member -> Patch.Replace(
            clockedDsn.clk.member,
            Patch.Replace.Config.ChangeRefAndRemove
          ))
          val rstPatch =
            if (hasPrevRst)
              List(
                prevDsn.rst.member -> Patch.Replace(
                  clockedDsn.rst.member,
                  Patch.Replace.Config.ChangeRefAndRemove
                )
              )
            else List()
          ...
          )) :: prevReplacements.flatMap(_.prevPatch)
        } else None
    }
...
object RTL {
  ...
  object IfBlock {
    ...
    )(implicit getSet: MemberGetSet): Option[(DFAny.Member, Boolean)] =
      cb match {
  ...
  implicit class Analysis(designDB: DFDesign.DB) {
    ...
    def caseWithDontCare(matchHeader: DFConditional.MatchHeader): Boolean = {
      ...
      refs.exists {
        case r: DFMember.OwnedRef =>
          r.refType match {
            case _: DFConditional.MatchHeader.Ref.Type =>
              r.owner.get match {
    ...
    def getSensitivityList(design: DFDesign.Block): List[String] = {
      val producers = designDB.designMemberTable(design).flatMap {
        case a @ DFNet.Assignment(_, fromVal, _, _) if !a.hasLateConstruction =>
          Some(fromVal)
        case c @ DFNet.Connection(_, fromVal, _, _) if !c.hasLateConstruction =>
          Some(fromVal)
        case DFAny.Func2.Unref(_, left, _, right, _, _) => List(left, right)
        case a: DFAny.Alias                             => Some(a.relValRef.get)
        case DFSimMember.Assert.Unref(condOption, msg, _, _, _) =>
          msg.seq ++ condOption
