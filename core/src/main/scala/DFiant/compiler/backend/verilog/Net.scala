    ...
    def unapply(net: DFNet)(implicit printer: Printer): Option[String] = {
      net match {
        case _: DFNet if net.isConnection && net.hasLateConstruction =>
          ...
          if (net.toRef.isMemberOfDesign(net.getOwnerDesign))
            Some(s".$toValStr${ALGN(0)}($fromValStr)")
  ...
  object Internal {
    def unapply(net: DFNet)(implicit printer: Printer): Option[String] = {
      ...
      net match {
        ...
        case _ =>
          val op =
            if (net.toRef.get.isTaggedWith(RTL.Tag.Mod.Reg)) "<="
            else "="
          (toVal, fromVal) match {
            case (
                  DFVector(_, _),
                  DFAny.Const(_, DFVector.Token(_, cellTokens), _, _)
                ) =>
              Some(
                cellTokens.zipWithIndex
                  .map {
                    case (v, i) =>
                      s"${toVal.name}[$i] ${ALGN(0)}$op ${Value.const(v)};"
                  }
                  .mkString("\n")
              )
            case (DFVector(_, cellNum), DFVector(_, _)) =>
              val iterName = "for_i"
              Some(
                s"""$KW for ($iterName = $LIT 0; $iterName < $LIT$cellNum; $iterName = $iterName + $LIT 1) $KW begin
