...
final class Compiler[D <: DFDesign](c: IRCompilation[D]) {
  private def isSyncMember(
      member: DFMember
  )(implicit printer: Printer): Boolean =
    member match {
      case RTL.IfBlock(_, _) | RTL.ElseIfBlock(_, _) => true
      case _                                         => false
    }

  private def getProcessStatements(
      block: DFBlock,
      filterFunc: DFMember => Boolean = _ => true
  )(implicit
      printer: Printer
  ): List[String] = {
    ...
    val simGuardSetOption: Option[String] =
      if (printer.inSimulation)
        Some(Verilator.ifndef(s"${Sim.guardName} = $LIT 1;"))
      else None
    val (_, statements) = printer.getSet.designDB
      .blockMemberTable(block)
      .filter(filterFunc)
      .foldRight(("", List.empty[String])) {
        case (
              cb @ DFConditional.IfElseBlock(None, Some(_), _, _),
              (_, statements)
            ) =>
        ...
        case (
              cb @ DFConditional.IfElseBlock(Some(condRef), Some(_), _, _),
              (closing, statements)
            ) =>
          (
            If.ElsIf(Value.ref(condRef.get), getProcessStatements(cb), closing),
            statements
          )
        case (cb @ RTL.IfBlock(RTL.IsClock(), _), (closing, statements)) =>
          ("", getProcessStatements(cb) ++ statements ++ simGuardSetOption)
        case (
              cb @ DFConditional.IfElseBlock(Some(condRef), None, _, _),
              (closing, statements)
            ) =>
          (
            "",
            If(
              Value.ref(condRef.get),
              getProcessStatements(cb),
              closing
            ) :: statements
          )
        case (
              cb @ DFConditional.CaseBlock(_, _, None, _, _),
              (_, statements)
            ) =>
          (
            Case.Item(Case.Choice.Default(), getProcessStatements(cb)),
            statements
          )
        case (
              cb @ DFConditional.CaseBlock(
                matchHeaderRef,
                _,
                Some(pattern),
                _,
          ...
          val item = Case.Item(
            Case.Choice.Pattern(pattern, width),
            getProcessStatements(cb)
          )
        ...
        case (mh: DFConditional.MatchHeader, (items, statements)) =>
          (
            "",
            Case(
              Value.ref(mh.matchValRef.get),
              items,
              printer.getSet.designDB.caseWithDontCare(mh)
            ) :: statements
          )
        case (Net.Internal(netStr), (closing, statements)) =>
          (closing, netStr :: statements)
        case (Sim.Assert(assertStr), (closing, statements)) =>
          (closing, assertStr :: statements)
        case (Sim.Finish(finishStr), (closing, statements)) =>
          (closing, finishStr :: statements)
        case (_, nochange) => nochange
      }
    statements
  }

  private def matchForceCover(
      getSet: MemberGetSet
  ): Iterable[DFConditional.MatchHeader] = {
    implicit val __getSet: MemberGetSet = getSet
    val matchHeaders = getSet.designDB.members.collect {
      case mh: DFConditional.MatchHeader => mh
    }
  ...
  def verilogCompile[R <: Revision](implicit revision: R) = {
    ...
    val files = designDB.designMemberList.flatMap {
      case (design: DFDesign.Block.Internal, _) if design.inlinedRep.nonEmpty =>
        None
      case (design: DFDesign.Block, members)
          if !designTypes.contains(design.designType) =>
        designTypes += design.designType
        val (ports, wires, regs) = members.foldRight(
          ...
          case (RTL.IsClock() | RTL.IsReset(), pwr)
              if design.isTop && printer.inSimulation =>
            pwr
          case (s: DFAny.Member, (ports, wires, regs)) if !s.isAnonymous =>
            designDB.getConnectionTo(s) match {
              case Some(n) if n.fromRef.isPortOut =>
                (
                  ports,
                  Wire(
                    s.name,
                    Type(s.dfType),
                    Type.arrayDim(s.dfType)
                  ) :: wires,
                  regs
                )
              case _ =>
        ...
        val clkrstPorts =
          if (design.isTop && printer.inSimulation)
            Verilator.ifdef(
              clkRstMembers
                .map(cr => Port(cr.name, Port.Dir.In(), Type(cr.dfType)))
                .mkString(",\n")
            ) :: ports
          else ports
        object ClkSim {
          def unapply(clk: DFAny.Member): Option[String] =
            clk match {
              case RTL.IsClock() =>
                val reg = Reg(clk.name, Type(clk.dfType), Init(clk))
                val sim =
                  s"$KW always #5 ${Value.ref(clk)} = $OP!${Value.ref(clk)};"
                Some(s"$reg\n$sim")
              case _ => None
            }
        }
        object RstSim {
          def unapply(rst: DFAny.Member): Option[String] =
            rst match {
              case RTL.IsReset() =>
                val reg = Reg(rst.name, Type(rst.dfType), Init(rst))
                val sim =
                  s"$KW initial #10 ${Value.ref(rst)} = $LIT${ResetParams.get.inactiveInt};"
                Some(s"$reg\n$sim")
              case _ => None
            }
        }
        val simGuardRegOption: Option[String] =
          if (printer.inSimulation && clkRstMembers.nonEmpty)
            Some(Verilator.ifndef(Reg(Sim.guardName, "", s" = $LIT 0")))
          else None
        val simGuardClearOption: Option[String] =
          if (printer.inSimulation && clkRstMembers.nonEmpty)
            Some(Verilator.ifndef(s"${Sim.guardName} = $LIT 0;"))
          else None
        ...
        val asyncStatements =
          getProcessStatements(design, !isSyncMember(_)) ++ simGuardClearOption
        val asyncSensitivityList: String = revision match {
          case Revision.V95 =>
            AlwaysBlock.Sensitivity(designDB.getSensitivityList(design))
        ...
        val modules =
          enumModuleDcls :+ Module(moduleName, clkrstPorts, declarations)
        val file =
          File(GlobalDefsFile.Name(), "", modules.mkString(s"\n$EMPTY\n"))
        Some(BackendStage.File(s"${moduleName}.v", s"$file"))
      case _ => None
    }
    val globalDefsFile =
      BackendStage.File(s"${GlobalDefsFile.Name()}.v", GlobalDefsFile())
    BackendStage
      .Compilation[D, Backend[R]](c.dsn, designDB, globalDefsFile :: files)
  }
}
