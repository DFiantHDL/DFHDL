  ...
  def const(token: DFAny.Token)(implicit printer: Printer): String = {
    ...
    token match {
      case t @ DFBits.Token(_, _) =>
        val hexRepOption: Option[String] =
...
//        value.map(const(_)).mkString("'{", ", ", "}")
  ...
  def alias(member: DFAny.Alias)(implicit printer: Printer): String = {
    ...
    member match {
      case DFAny.Alias.AsIs(dfType, _, _, _, _) =>
        dfType match {
        ...
        if (relWidth == relVal.width) relValStr
          ...
          dfType match {
            case DFBool.Type(false) =>
              s"${relValStr.applyBrackets()}[$LIT$relBitLow]"
            case DFBits.Type(_) =>
              s"${relValStr.applyBrackets()}[$LIT$relBitHigh:$LIT$relBitLow]"
          }
        }
