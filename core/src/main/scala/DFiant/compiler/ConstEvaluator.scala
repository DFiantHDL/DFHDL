...
object ConstEvaluator extends ElementEvaluator[DFAny.Token] {
  def applyElement(token : DFAny.Token, element : SourceElement) : DFAny.Token = {
    ...
    val tokenReverse = if (element.reversed) tokenInvert.reverse else tokenInvert
    element.dfType.getTokenFromBits(tokenReverse)
  }

  private def concat(dfType : DFAny.Type, elements : List[DFAny.Token]) : DFAny.Token =
    elements match {
      case head :: Nil => head
  ...
  def apply(element : SourceElement)(
    implicit evaluationMap : EvaluationMap[DFAny.Token], dependencyContext : DependencyContext
  ) : DFAny.Token = {
      ...
      case SourceValue.Dcl(dcl, version) =>
        version match {
          ...
          case SourceVersion.Idx(block, idx) =>
            concat(
              element.dfType,
              dependencyContext.assignmentMap(dcl)(block)(idx)._2.elements.map(evaluationMap.evaluate)
          ...
          case SourceVersion.IfElse(_, branchVersions, fallbackVersion) =>
            val fallBackToken = evaluationMap.evaluate(element.versioned(dcl, fallbackVersion))
            val ifElseToken = branchVersions.foldRight(fallBackToken){ case ((condSrc, thenVersion), elseToken) =>
              ...
              val thenToken = evaluationMap.evaluate(element.versioned(dcl, thenVersion))
              condToken match {
