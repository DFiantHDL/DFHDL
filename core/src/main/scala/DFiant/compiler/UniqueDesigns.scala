...
private final class UniqueBlock(
    val block: DFDesign.Block.Internal,
    val members: List[DFMember]
)(implicit getSet: MemberGetSet) {
  override def equals(obj: Any): Boolean =
    obj match {
      case that: UniqueBlock
          if this.block.designType == that.block.designType =>
...
final class UniqueDesigns[D <: DFDesign](c: IRCompilation[D]) {
  ...
  def uniqueDesigns: IRCompilation[D] = {
    val uniqueBlockMap: Map[UniqueBlock, List[DFDesign.Block.Internal]] =
      designDB.members
        .collect {
          case block: DFDesign.Block.Internal if block.inlinedRep.isEmpty =>
            block
        }
        .groupBy(b => new UniqueBlock(b, designDB.designMemberTable(b)))
    val uniqueTypeMap: Map[String, List[UniqueBlock]] =
      uniqueBlockMap.keys.toList.groupBy(ub => ub.block.designType)
        ...
        list.zipWithIndex.flatMap {
          case (ub, i) =>
            ...
            uniqueBlockMap(ub).map(block =>
              block -> Patch.Replace(
                block.copy(designType = updatedDesignType),
