...
final class Pipelining[D <: DFDesign](c: IRCompilation[D]) {
  ...
  abstract class PipeDesign(implicit meta: internals.Meta)
      extends MetaDesign() {
    def addPipes(member: DFAny.Member, extraPipe: ExtraPipe): DFAny.Member =
      if (extraPipe.extraStages > 0)
        member.asVal.pipe[Int](extraPipe.extraStages).anonymize
      else member

    def addPipes(
        member: DFAny.Member,
        extraPipes: List[ExtraPipe]
    ): DFAny.Member =
      extraPipes match {
        ...
        case _ if !member.isAnonymous =>
          ...
          val piped = pipedParts
            .map(p => p.asValOf[DFBits.Type[Int]])
            .reduce(_ ++ _)
            .anonymize
          member.dfType match {
  ...
  private def addPipelines(
    ...
    val pipeDelayMap = designDB.evaluate(
      new PipelineEvaluator(pipelineMethod, pipeDelayEstimator)
    )
    val patchList = pipeDelayMap.flatMap {
      case (
            SourceValue.Func2(dfVal),
            PipelineInfo.Func2(
              width,
              leftExtraPipes,
              rightExtraPipes,
        ...
        if (
          leftExtraPipes.pipeRequired || rightExtraPipes.pipeRequired || internalExtraPipe > 0
        ) {
//          println(leftExtraPipes, rightExtraPipes, internalExtraPipe)
          ...
          val dsn = new PipeDesign() {
            ...
            addPipes(
              updatedFunc,
              List(ExtraPipe(updatedFunc.width, internalExtraPipe))
            ).setTags(_ => dfVal.tags)
          }
