...
final class Flatten[D <: DFDesign](c: IRCompilation[D]) {
  ...
  private def flattenName(member: DFMember): DFMember =
    member.setName(s"${member.getOwnerBlock.name}_${member.name}")
  private def flattenPort(port: DFAny.Member): List[(DFMember, Patch)] = {
    ...
    if (producersToPort.size == 1) {
      ...
      val ibMembers =
        designDB.blockMemberTable(
          incomingBlock
        ) //TODO: perhaps at any hierarchy?
      val unusedNet = ibMembers.collectFirst {
        case m: DFNet if m.isConnection && m.toRef.get == port => m
      }.get
      val replacement = if (producerToPort.isAnonymous) {
        if (designDB.producerDependencyTable(producerToPort).size > 1)
          producerToPort.setName(port.name)
        else producerToPort
      } else producerToPort
      List(
        (
          port: DFMember,
          Patch.Replace(replacement, Patch.Replace.Config.FullReplacement)
        ),
        (unusedNet, Patch.Remove)
      )
    } else {
      List(
        port -> Patch.Replace(
    ...
    if (block.isTop) List()
      ...
      (block -> Patch.Replace(
        ...
        case m if !m.isAnonymous =>
          List(
            m -> Patch
              .Replace(flattenName(m), Patch.Replace.Config.FullReplacement)
          )
        case _ => None
      }
    }
  ...
  def flattenInline: IRCompilation[D] = {
    val inlineBlocks = designDB.members.collect {
      case ib @ DFDesign.Block.Internal(_, _, _, Some(_)) => ib
    }
  ...
  def flatten(design: DFDesign*): DFDesign.DB =
    designDB.patch(design.flatMap(d => flattenPatch(d.owner)).toList)
}
