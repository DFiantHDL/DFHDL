...
final class FlattenNames[D <: DFDesign](c: IRCompilation[D]) {
  ...
  @tailrec private def recursiveNameFlatten(
      member: DFMember,
    ...
    member.getOwner match {
      case o: DFOwner.NameFlattenOwner =>
        o.nameFlatten match {
          case DFOwner.NameFlatten.IgnoreOwnerName =>
            recursiveNameFlatten(o, nameFunc)
          case _ =>
            recursiveNameFlatten(o, n => o.nameFlatten(nameFunc(n), o.name))
        }
      case _ => nameFunc
    }
  private def recursiveNameFlatten(member: DFMember): String => String =
    recursiveNameFlatten(member, n => n)
  def flattenNames: IRCompilation[D] = {
    val (patchList, tagList) = designDB.members.foldRight(
      (
        List.empty[(DFMember, Patch)],
        List.empty[((Any, ClassTag[_]), DFMember.CustomTag)]
      )
    ) {
          ...
          case _: DFOwner.NameFlattenOwner =>
            ...
            val updatedTags = m match {
              case DFAny.Dcl(
                ...
                val tag =
                  (enumType, classTag[EnumType.NameTag]) -> EnumType.NameTag(
                    updatedNameFunc(enumType.name)
                  )
                tag :: ret._2
              case _ =>
                ret._2
