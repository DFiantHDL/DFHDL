...
final class InitCalc[D <: DFDesign](c: IRCompilation[D]) {
  ...
  @tailrec private def initCalc(
      ...
      calc: Map[DFAny.Member, Seq[DFAny.Token]],
      requestedCalc: Set[DFAny.Member]
  ): Map[DFAny.Member, Seq[DFAny.Token]] = {
    def getInit(member: DFAny.Member): Option[Seq[DFAny.Token]] =
...
//    }
    remaining match {
      case m :: mList =>
        if (getInit(m).isDefined) initCalc(mList, calc, requestedCalc)
        else
          m match {
            case CalcDep(deps, func) =>
              val initOptions = deps.flatMap(d => getInit(d))
              if (initOptions.length == deps.length)
                initCalc(mList, calc + (m -> func(initOptions)), requestedCalc)
              else initCalc(deps ++ remaining, calc, requestedCalc + m)
            case v: DFAny.Member =>
              designDB.getConnectionTo(v) match {
                //uses connection init
                case Some(n) =>
                  val s = n.fromRef.get
                  getInit(s) match {
                    case Some(init) =>
                      initCalc(mList, calc + (m -> init), requestedCalc)
                    ...
                    case None =>
                      initCalc(s :: remaining, calc, requestedCalc + m)
                  }
  ...
  def initCalc: IRCompilation[D] = {
    ...
    val calcMembers = designDB.members.collect {
      case v: DFAny.Member if v.getInit.isEmpty => v
    }
  ...
  def clearInitCalc: IRCompilation[D] = {
    val calcMembers = designDB.members.collect {
      case v: DFAny.Member if v.getInit.nonEmpty => v
    }
