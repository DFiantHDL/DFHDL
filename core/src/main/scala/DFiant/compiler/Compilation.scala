...
final case class IRCompilation[D <: DFDesign](dsn: D, db: DFDesign.DB)
    extends Compilation[D] {
  def newStage(updatedDB: DFDesign.DB): IRCompilation[D] =
    IRCompilation[D](dsn, updatedDB)
}
...
object Compilation {
  ...
  implicit def fromCompilation[D <: DFDesign]
      : Compilation[D] => IRCompilation[D] =
