...
//      designDB.dependencyContext.stateVars.groupBy(_.getOwnerDesign)
    ...
    @tailrec def getControlledOwners(
        current: DFDesign.Block,
        list: List[DFDesign.Block]
    ): List[DFDesign.Block] = {
    ...
    val patchList: List[(DFMember, Patch)] = designDB.designMemberList.flatMap {
      case (ownerDesign, members)
          if controlledStateDesigns.contains(ownerDesign) || controllingDesigns
            .contains(ownerDesign) =>
        verbose {
          println("-----------------")
        ...
        val stateVarList = members.collect {
          case v: DFAny.Dcl if stateVarSet.contains(v) => v
        }
        val controlledStateDesignMembers = members.collect {
          case d: DFDesign.Block if controlledStateDesigns.contains(d) => d
        }
        val controlledDesignMembers = members.collect {
          case d: DFDesign.Block if controlledDesigns.contains(d) => d
        }
        val controllers = members.collect { case c: DFDesign.Control => c }

        //Adding control port for all designs that require it
        val controlPortPatchOption =
          if (controlledStateDesigns.contains(ownerDesign)) {
            verbose {
              println(s"Adding ${ownerDesign.getFullName} dsnCtrl port")
            }
        ...
        val designMemberPatchList = controlledStateDesignMembers.map {
          ...
          case md =>
            verbose {
              println(
                s"Connecting ${md.getFullName}.dsnCtrl <> ${ownerDesign.getFullName}.dsnCtrl"
              )
            }
            val dsn = new MetaDesign() {
        ...
        val statePatchOption = controlPortMap.get(ownerDesign) match {
          case Some(dsnCtrl)
              if stateVarList.nonEmpty || controlledDesignMembers.nonEmpty =>
            verbose {
              println(s"Handling state vars:", stateVarSet.map(_.name))
              println(
                s"Handling control vars for:",
                controlledDesignMembers.map(_.name)
              )
            }
            val dsn = new MetaDesign() {
              matchdf(dsnCtrl)
                ...
                .casedf(Op.Init) {
                  stateVarList.foreach { v =>
                    v.externalInit match {
                      case Some(token +: _) =>
                        v.asVar := DFAny.Const.forced[DFAny.Type](token)
                      case _ => v.asVar := ?
                    }
                  }
