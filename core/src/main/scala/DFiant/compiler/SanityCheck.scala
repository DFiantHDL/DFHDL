...
final class SanityCheckOps[D <: DFDesign](c : IRCompilation[D]) {
    ...
    val violations = designDB.members.flatMap {
      case n @ DFNet.Assignment.Unref(toVal, fromVal, _,_) =>
        ...
        if (toValMissing) {
          ...
          designDB.members.collectFirst { case m if m.name == toVal.name => m } match {
        ...
        if (fromValMissing) {
          ...
          designDB.members.collectFirst { case m if m.name == fromVal.name => m } match {
            case Some(value) => println(s"Found:\n$value\nInstead of:\n$fromVal")
            case None =>
          }
        }
  ...
  @tailrec private def ownershipCheck(currentOwner : DFOwner, members : List[DFMember]) : Unit = {
    members match {
      case m :: nextMembers if (m.getOwner == currentOwner) => m match { //still in current owner
      ...
      case m :: _ => //not in current owner
        if (currentOwner.isTop) {
          println(s"The member ${m.hashCode().toHexString}:\n$m\nHas owner ${m.getOwner.hashCode().toHexString}:\n${m.getOwner}")
          val idx = designDB.members.indexOf(m)
          val prevMember = designDB.members(idx-1)
          println(s"Previous member ${prevMember.hashCode().toHexString}:\n$prevMember\nHas owner ${prevMember.getOwner.hashCode().toHexString}:\n${prevMember.getOwner}")
