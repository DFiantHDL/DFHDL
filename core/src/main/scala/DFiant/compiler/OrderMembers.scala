  ...
  object Order {
    implicit val Simple : Order = new Order {
      def apply(implicit getSet : MemberGetSet) : DFMember => Int = {
        ...
        case DFNet.Connection.Unref(_, DFAny.In(), _, _) => 4
        case DFNet.Connection.Unref(DFAny.NewVar(), v @ DFAny.NewVar(), _, _) if !v.isNonAliasAssigned => 4
        case DFNet.Connection.Unref(DFAny.Out(), _, _, _) => 6
        case DFNet.Assignment.Unref(v @ DFAny.Out(), _, _, _) if getSet.designDB.getAssignmentsTo(v).size == 1 => 6
        case _ => 5
      }
    }
