...
object PipelineEvaluator {
  ...
  final case class DelayEstimation(
      consumeDelay: Int,
      maxDelay: Int,
      produceDelay: Int
  )
  ...
  trait PipeDelayEstimator {
    def func2(func2: DFAny.Func2)(implicit
        getSet: MemberGetSet
  ...
  implicit object DefaultPipeDelayEstimator extends PipeDelayEstimator {
    ...
    private def gateLogicDelay(
      ...
      val LUTCount =
        (((funcInputWidth - 1) / LUTInputs) max ((funcOutputWidth - 1) / LUTOutputs)) + 1
      val SliceCount = (LUTCount - 1) / LUTsPerSlice + 1
      val delay =
        funcDepth * ((LUTCount - 1) * LUTtoLUTPenalty + (SliceCount - 1) * SliceToSlicePenalty + LUTCount * LUTDelay)
      ...
      if (maxPipe <= 2) basicEstimations
        ...
        basicEstimations ++ Vector.tabulate(maxPipe - 2)(i =>
          DelayEstimation(0, delay / (1 << i + 1), 0)
        )
    }
    ...
    def func2(
        func2: DFAny.Func2
    )(implicit getSet: MemberGetSet): PipeDelayEstimations = {
      val maxWidth = func2.leftArgRef.get.width max func2.rightArgRef.get.width
      val leftIsConstant =
        func2.leftArgRef.get match { //TODO: include constant propagation analysis
          case _: DFAny.Const => true
          case _              => false
        }
      val rightIsConstant =
        func2.rightArgRef.get match { //TODO: include constant propagation analysis
          case _: DFAny.Const => true
          case _              => false
        }
      if (leftIsConstant && rightIsConstant) noDelay
        ...
        func2.op match {
          case Op.+ | Op.- =>
            gateLogicDelay(maxWidth * 2, maxWidth, 1, maxWidth.bitsWidth(false))
          case Op.+^ | Op.-^ =>
            gateLogicDelay(
              (maxWidth + 1) * 2,
              maxWidth + 1,
              1,
              maxWidth.bitsWidth(false)
            )
          case Op.* =>
            gateLogicDelay(
              maxWidth * 2,
          ...
          case Op.== | Op.!= =>
            gateLogicDelay(2, 1, maxWidth.bitsWidth(false), 2)
          case Op.< | Op.> | Op.<= | Op.>= =>
            gateLogicDelay(maxWidth * 2, maxWidth, 1, 2)
          case Op.| | Op.& | Op.|| | Op.&& | Op.^ => gateLogicDelay(2, 1, 1, 2)
          case shift: Op.Shift =>
            if (rightIsConstant) noDelay
            else
              gateLogicDelay(
                maxWidth * 2,
                maxWidth,
