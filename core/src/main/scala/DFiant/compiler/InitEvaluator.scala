...
object InitEvaluator extends ElementEvaluator[Seq[DFAny.Token]] {
  def applyElement(
    ...
    if (!element.withInit) Seq()
      ...
      val tokenInvertSeq =
        if (element.inverted) DFAny.TokenSeq(tokenBitsSeq)(~_) else tokenBitsSeq
      val tokenReverseSeq =
        if (element.reversed) DFAny.TokenSeq(tokenBitsSeq)(_.reverse)
        else tokenInvertSeq
  ...
  def apply(element: SourceElement)(implicit
      evaluationMap: EvaluationMap[Seq[DFAny.Token]],
            ...
            case None =>
              val elements = version match {
                ...
                case SourceVersion.Idx(block, idx) =>
                  dependencyContext.assignmentMap(dfVal)(block)(idx)._2.elements
              ...
              concat(element.dfType, elements.map(evaluationMap.evaluate))
