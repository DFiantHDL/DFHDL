...
trait ElementEvaluator[T] { self =>
  ...
  def getDependencies(element: SourceElement)(implicit
      dependencyContext: DependencyContext
...
final class PathEvaluator[D <: DFDesign](c: IRCompilation[D]) {
  ...
  @tailrec private def evaluate[T](
    ...
    stack match {
      ...
      case (currentElement :: next, pendingTable) :: nextStack
          if evaluationMap.contains(currentElement.srcVal) =>
        if (elementEvaluator.verboseEvaluation)
          println("already evaluated", currentElement.codeString)
          ...
          //we just discovered
            ...
            .map {
              case (element, _) =>
                (
                  element.srcVal,
                  elementEvaluator.cyclic(element)(evaluationMap, dc)
                )
            }
        if (elementEvaluator.verboseEvaluation)
          println("cyclic dependency found", currentDependency.codeString)
        evaluate(
          (next, pendingTable) :: nextStack,
          evaluationMap ++ evaluatedCyclicElements
        )
      ...
      case (Nil, _) :: (currentElement :: next, pendingTable) :: nextStack
          if evaluationMap.contains(currentElement.srcVal) =>
        if (elementEvaluator.verboseEvaluation)
          println("finished cyclic path at", currentElement.codeString)
        evaluate((next, pendingTable) :: nextStack, evaluationMap)
      ...
      case (Nil, _) :: (currentElement :: next, pendingTable) :: nextStack =>
        ...
        val evaluatedElement =
          elementEvaluator(currentElement)(evaluationMap, dc)
        evaluate(
          (next, pendingTable) :: nextStack,
          evaluationMap.updated(currentElement.srcVal, evaluatedElement)
        )
      ...
      case (currentElement :: _, pendingTable) :: _ =>
        ...
        if (elementEvaluator.verboseEvaluation)
          println(
            "waiting for dependencies for",
            currentElement.codeString,
            ":",
            dependencies.map(_.codeString)
          )
  ...
  def printEvaluation[T](implicit
    ...
    println(
      ListSet
        .from(dependencyMap.values.flatMap(_.elements))
        .map(src =>
