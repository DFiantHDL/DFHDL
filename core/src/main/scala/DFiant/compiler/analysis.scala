...
object analysis {
  final implicit class DFAnyAnalysis(value : DFAny.Member)(implicit getSet: MemberGetSet) {
    ...
    @tailrec private def suggestName(member : DFAny.Member, prevMember : Option[DFAny.Member] = None) : String = {
  ...
  final implicit class DFMemberAnalysis(member : DFMember)(implicit getSet: MemberGetSet) {
    @tailrec private def getCBOwners(
      currentOwner : DFBlock, list : List[DFConditional.Block]
    ) : List[DFConditional.Block] =
      currentOwner match {
        case _ : DFDesign.Block => list
        case cb : DFConditional.Block => getCBOwners(currentOwner.getOwnerBlock, cb :: list)
      }
  ...
  final implicit class IfBlockAnalysis(ifBlock : DFConditional.IfElseBlock)(implicit getSet: MemberGetSet) {
    ...
    def getBranches : Iterable[IfElseBlock] = {
      ...
      firstIfBlock ::
        getSet.designDB.blockMemberTable(firstIfBlock.getOwnerBlock).dropWhile(_ != firstIfBlock).drop(1).takeWhile {
  ...
  final implicit class MatchHeaderAnalysis(mh : DFConditional.MatchHeader)(implicit getSet: MemberGetSet) {
  ...
  final implicit class ConditionalBlockAnalysis(cb : DFConditional.Block)(implicit getSet: MemberGetSet) {
    @tailrec def getFirstCB : DFConditional.Block = cb.prevBlockRefOption match {
    ...
    @tailrec private def getPatterns(casePattenBlock : DFConditional.CaseBlock, patterns : List[DFAny.Pattern]) : List[DFAny.Pattern] = {
    ...
    @tailrec private def getLeadingCBChain(
      cb : DFConditional.Block, chain : List[DFConditional.Block]
    ) : List[DFConditional.Block] =
    ...
    @tailrec private def getTopConditionalMember(currentBlock : DFConditional.Block) : DFMember =
      currentBlock.getOwnerBlock match {
        case cb : DFConditional.Block => getTopConditionalMember(cb)
        case _ => currentBlock match {
          case DFConditional.IfElseBlock(_,Some(prevBlockRef),_,_) => prevBlockRef.get
