  ...
  class VHDL(design : DFInterface, owner : VHDL = null, simClkPeriodKHz : Option[Int] = None) extends Backend(design) { self =>
    ...
    protected object Type {
      ...
      def apply(member : DFAny) : Type = member match {
        ...
        case _ => throw new IllegalArgumentException(s"\nUnsupported type for VHDL compilation. The variable ${member.fullName} has type ${member.__dslMemberFields.typeName}")
      }
...
//              s"${Value(aliasedVar).value.applyBrackets()} $op $shift"
        ...
        case class component(rtComponent: RTComponent) extends PortsHandler(rtComponent, 2) {
          ...
          override def toString: String = s"\n${delim}component ${rtComponent.__dslMemberFields.typeName.toLowerCase}${ports}\n${delim}end component;"
...
//        else
    ...
    val entityName : Name = if (design.isInstanceOf[RTComponent]) Name(design.__dslMemberFields.typeName.toLowerCase) else {
      pass(design)
      architecture.statements.async_process.variables.toSigPorts
      val topOrElseName = if (design.isTop) design.name else design.__dslMemberFields.typeName
