...
final case class DependencyContext(
        ...
        val assignedSrc =
          getLatestSource(dcl) //getting the last assignment or an empty source
            .assignWL(
              ...
              relBitLow,
              withInit
            ) //modifying the related bits with the new source
        val blockVersions = assignmentMap.getOrElse(dcl, ListMap())
        val versions =
          blockVersions.getOrElse(currentBlock, Vector()) :+ (net, assignedSrc)
        copy(assignmentMap =
          assignmentMap
            .updated(dcl, blockVersions.updated(currentBlock, versions))
        )
    }
  }

  protected[DependencyContext] def assignWith(net: DFNet)(implicit
      currentBlock: DFBlock
  ): DependencyContext = {
  ...
  protected[DependencyContext] def branchExit(
      exitBlock: DFConditional.Block
  ): DependencyContext = {
    if (exitBlock.isLastCB) {
      ...
      val updatedAssignmentMap = assignmentMap.foldLeft(assignmentMap) {
        case (assignmentMap, (dcl, blockVersions)) =>
          exitBlock match {
            case ifExitBlock: DFConditional.IfElseBlock =>
              val branches = blockVersions.view.keys.collect {
                case cb: DFConditional.IfElseBlock => cb
              }
              if (cbSet.intersectsWith(branches)) {
                ...
                val fallBackVersion =
                  if (ifExitBlock.condRefOption.isEmpty)
                    blockVersions.getLatestVersionIn(ifExitBlock)
                  else blockVersions.getLatestVersionIn(ownerBlock)
                val headBranch = branches.head
                val src = Source.IfElse(
                  dcl,
                  headBranch,
                  branchVersions,
                  fallBackVersion,
                ...
                val ownerVersions = blockVersions.getOrElse(
                  ownerBlock,
                  Vector()
                ) :+ (headBranch, src)
                assignmentMap.updated(
                  dcl,
                  blockVersions.updated(ownerBlock, ownerVersions)
                )
              } else assignmentMap
            case caseExitBlock: DFConditional.CaseBlock =>
              val cases = blockVersions.view.keys.collect {
                case cb: DFConditional.CaseBlock => cb
              }
              if (cbSet.intersectsWith(cases)) {
                ...
                val ownerBlock = exitBlock.getOwnerBlock
                val fallBackVersion =
                  if (caseExitBlock.patternOption.isEmpty)
                    Some(blockVersions.getLatestVersionIn(caseExitBlock))
                  else if (!caseExitBlock.isExhaustive)
                    Some(blockVersions.getLatestVersionIn(ownerBlock))
                  else None
          ...
          case (block, versions) =>
            ...
            if (versions.size > 1 || isCB)
              versions.zipWithIndex.foreach {
                case ((_, source), i) =>
                  val n =
                    SourceValue.Dcl(dcl, SourceVersion.Idx(block, i)).codeString
                  println(f"$n%-40s ${source.codeString}")
              }
            else
              println(f"${dcl.getFullName}%-40s ${versions.head._2.codeString}")
        }
    }
    this
  }
  ...
  protected[DependencyContext] def dontConsume(
      fromVal: DFAny.Member
  ): DependencyContext = ???
  protected[DependencyContext] def consume(
      fromVal: DFAny.Member
  ): DependencyContext = this
}
...
//  type CBVersionMap = Map[(DFAny.Dcl, DFConditional.Block), Int]
      ...
      case (nextBlock: DFBlock) :: rs
        ...
        val updatedDependency: DependencyContext = nextBlock match {
          ...
          case cb: DFConditional.IfElseBlock if cb.isFirstCB =>
            ...
            condRefs
              .foldLeft(dependencyContext)(_.consumeFrom(_))
              .branchEntry(cb)
        ...
        val updatedDependency: DependencyContext = r match {
          ...
          case assert: DFSimMember.Assert =>
            ...
            assert.msgRef.seq
              .collect {
                case Left(x) => x
              } //get just the reference to dataflow values
          ...
          case DFAny.Dynamic.DontProduce(relVal, _, _) =>
            dependencyContext.dontProduce(relVal)
          case DFAny.Dynamic.DontConsume(relVal, _, _) =>
            dependencyContext.dontConsume(relVal)
          case DFAny.Dynamic.Consume(relVal, _, _) =>
            dependencyContext.consume(relVal)
      ...
      case _ => //exiting child block or no more members
        val updatedDependency = currentBlock match {
          case d: DFDesign.Block if remaining.isEmpty =>
            val outPorts: List[DFAny.Member] =
              getSet.designDB.designMemberTable(d).collect {
        ...
        val exitingBlock = remaining match {
          ...
          case Nil if (currentBlock != getSet.designDB.top) =>
        ...
        if (exitingBlock) {
          ...
          getDependencies(remaining, updatedDependency)(
            currentBlock.getOwnerBlock
          )
  ...
  def apply(designDB: DFDesign.DB): DependencyContext = {
    ...
    getDependencies(designDB.members.drop(1), DependencyContext.empty)(
      designDB.top
    )
  }
}
