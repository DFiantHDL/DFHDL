...
final case class DependencyContext(
  ...
  @tailrec private def assignTo(
    ...
    toVal match {
      ...
      case DFAny.Alias.Invert(_,rv,_,_) => assignTo(rv.get, net, relWidth, relBitLow, fromSrc.invert)
  ...
  def getLatestSource(dcl : DFAny.Dcl)(implicit currentBlock : DFBlock) : Source =
    assignmentMap.get(dcl) match {
      case Some(blockVersions) => Source(dcl, blockVersions.getLatestVersionIn(currentBlock), withInit = true)
        ...
        case (block, versions) =>
          ...
          if (versions.size > 1 || isCB)
            versions.zipWithIndex.foreach {
              case ((_, source), i) =>
                val n = SourceValue.Dcl(dcl, SourceVersion.Idx(block, i)).codeString
  ...
  lazy val initMap : EvaluationMap[Seq[DFAny.Token]] = getSet.designDB.evaluate(InitEvaluator)
  lazy val constMap : EvaluationMap[DFAny.Token] = getSet.designDB.evaluate(ConstEvaluator)
