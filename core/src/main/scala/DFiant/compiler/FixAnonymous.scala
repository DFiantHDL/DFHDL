...
final class FixAnonymous[D <: DFDesign](c: IRCompilation[D]) {
  ...
  def dropUnreferenced: IRCompilation[D] = {
    val patchList = designDB.members.collect {
      case m @ DFAny.NewVar()
          if !designDB.memberTable.contains(m) && m.externalInit.isEmpty =>
        m -> Patch.Remove
    }
  ...
  def fixAnonymous: IRCompilation[D] = {
    val anonymizeList = designDB.designMemberList.flatMap {
      case (block, members) =>
        ...
        members
          ...
          .groupBy(m => m.tags.meta.namePosition)
          .flatMap {
    ...
    val patchList = anonymizeList.map(a =>
      a -> Patch.Replace(a.anonymize, Patch.Replace.Config.FullReplacement)
    )
    c.newStage(designDB.patch(patchList))
  }
}
