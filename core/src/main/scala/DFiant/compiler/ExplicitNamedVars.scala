  ...
  def explicitNamedVars : IRCompilation[D] = {
    val patchList = designDB.members.flatMap {
      case named : DFAny.Member if !named.isAnonymous => named match { //all named values
        ...
        case _ : DFAny.Const => None //named constants remain as they are
        case _ : DFAny.ApplySel => None //array selection
        case _ =>
            ...
            case cb : DFConditional.Block => //inside a conditional block
              val dsnNewVar = new MetaDesign() {
                ...
                if (externalInit.isDefined && named.requiresDefaultInit) {
                  plantedNewVar.assign(DFAny.Const.forced(externalInit.get.head))
                }
              }
