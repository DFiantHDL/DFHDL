...
final class ConvertMatchToIf[D <: DFDesign](c : IRCompilation[D]) {
  ...
  def convertMatchToIf(matchHeaders : MemberGetSet => Iterable[DFConditional.MatchHeader]) : IRCompilation[D] = {
    val patchList = matchHeaders(designDB.__getset).flatMap { mh =>
      ...
      val casePatches =
        (cases.lazyZip(condDsn.conds)).foldLeft[(List[(DFMember, Patch)], Option[DFConditional.IfElseBlock])](List(), None) {
          case ((patches, prevIfOption), (mc, condOption)) =>
            val dsn = new MetaDesign() {
              val ifElseBlock = DFConditional.IfElseBlock(condOption, prevIfOption)
            }
