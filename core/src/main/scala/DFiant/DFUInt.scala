...
object DFUInt extends DFAny.Companion {
  ...
  def max[U](maxValue: Positive.Checked[U])(implicit
      ctx: DFAny.Context,
      w: BitsWidthOf.Int[U]
  ): DFAny.NewVar[Type[w.Out]] = DFAny.NewVar(Type(w(maxValue.getValue)))
  def until[U](supremum: Positive.Checked[U])(implicit
      ctx: DFAny.Context,
      w: BitsWidthOf.Int[U - 1]
  ...
  final case class Token(width: Int, value: Option[BigInt])
    ...
    private def mkTokenU(
      ...
      (left.value, right.value) match {
        case (Some(l), Some(r)) =>
          Token(resultWidth, f(l, r).asUnsigned(resultWidth))
        case _ => Token.bubble(resultWidth)
      }
    }

    def +(right: Token): Token =
      mkTokenU(right, _ + _, left.width max right.width)
    def +^(right: Token): Token =
      mkTokenU(right, _ + _, (left.width max right.width) + 1)
    def -(right: Token): Token =
      mkTokenU(right, _ - _, left.width max right.width)
    def -^(right: Token): Token =
      mkTokenU(right, _ - _, (left.width max right.width) + 1)
    def *(right: Token): Token =
      mkTokenU(right, _ * _, left.width max right.width)
    def *^(right: Token): Token =
      mkTokenU(right, _ * _, left.width + right.width)
  ...
  object Token {
    ...
    def apply(width: Int, value: BigInt): Token = {
      if (value < 0)
        throw new IllegalArgumentException(
          s"Unsigned token value must not be negative. Found $value"
        )
      assert(
        value.bitsWidth(false) <= width,
    ...
    trait Implicits {
      implicit def __DFUIntTokenAsIsInt[LW, V <: Int, OW](implicit
      ...
      implicit def __DFUIntTokenAsIsLong[LW, V <: Long, OW](implicit
      ...
      implicit def __DFUIntTokenAsIsBigInt[LW]: AsIs[LW, BigInt, Int] =
      ...
      implicit def __DFUIntTokenToFit[LW, V, RW](implicit
  ...
  object Pattern extends PatternCO {
    ...
    object Able {
      ...
      implicit class DFUIntPatternBigInt[R <: BigInt](val right: R)
          extends Able[R] {
        val interval: Interval[BigInt] = Interval.point(right)
      }
      implicit class DFUIntPatternRange[R <: Range](val right: R)
          extends Able[R] {
        val interval: Interval[BigInt] =
          Interval.fromRange(right).toBigIntInterval
      }
      implicit class DFUIntPatternIntervalInt[R <: Interval[Int]](val right: R)
          extends Able[R] {
        val interval: Interval[BigInt] = right.toBigIntInterval
      }
      implicit class DFUIntPatternIntervalLong[R <: Interval[Long]](
          val right: R
      ) extends Able[R] {
        val interval: Interval[BigInt] = right.toBigIntInterval
      }
      implicit class DFUIntPatternIntervalBigInt[R <: Interval[BigInt]](
          val right: R
      ) extends Able[R] {
        ...
        new Builder[Type[LW]] {
          def apply[R](left: Type[LW], right: Seq[Able[R]]): Pattern = {
            val reqInterval = IntervalSet(
              Interval
                .closed(BigInt(0), BigInt.maxUnsignedFromWidth(left.width))
            )
  ...
  object Const {
    ...
    object PosNeg {
      ...
      implicit def fromValueOf[N](implicit
        ...
        new PosNeg[ValueOf[N]] {
          type W = const.W
          def apply(value: ValueOf[N]): (DFUInt[W], Boolean) =
            const(value.value)
        }
      implicit def fromInt[N <: Int](implicit
          ctx: DFAny.Context,
          w: BitsWidthOf.Int[Abs[N]],
          di: DummyImplicit
      ): Aux[N, w.Out] =
        new PosNeg[N] {
          type W = w.Out
          def apply(value: N): (DFUInt[W], Boolean) = {
            ...
            (
              DFAny.Const[Type[W]](Type(width), Token(width, absValue)),
              value < 0
            )
          }
        }
      implicit def fromLong[N <: Long](implicit
          ctx: DFAny.Context,
          w: BitsWidthOf.Long[Abs[N]],
          di: DummyImplicit
      ): Aux[N, w.Out] =
        new PosNeg[N] {
          type W = w.Out
      ...
      implicit def fromBigInt[N <: BigInt](implicit
          ctx: DFAny.Context
      ): Aux[N, Int] =
        new PosNeg[N] {
          type W = Int
  ...
  object Op {
    ...
    class AbleOps[L](value: L) extends Able[L](value) {
      val left = value
      final def +[RW](right: DFUInt[RW])(implicit
          op: `Op+`.Builder[L, false, DFUInt[RW]]
      ) = op(left, right)
      final def +^[RW](right: DFUInt[RW])(implicit
          op: `Op+^`.Builder[L, true, DFUInt[RW]]
      ) = op(left, right)
      final def -[RW](right: DFUInt[RW])(implicit
          op: `Op-`.Builder[L, false, DFUInt[RW]]
      ) = op(left, right)
    ...
    trait Implicits {
      final implicit def __DFUIntWiden[FW, TW](c: DFUInt[FW])(implicit
          eq: OpContainer.Eq[FW, TW, Int]
      ): DFUInt[TW] = c.asInstanceOf[DFUInt[TW]]
      sealed class __DFUIntFromInt[L <: Int](left: L) extends AbleOps[L](left)
      final implicit def __DFUIntFromInt[L <: Int](
          left: L
      ): __DFUIntFromInt[L] = new __DFUIntFromInt(left)
      sealed class __DFUIntFromXInt[L <: XInt](left: L)
          extends AbleOps[ValueOf[L]](new ValueOf(left))
      final implicit def __DFUIntFromXInt[L <: XInt](
          left: L
      ): __DFUIntFromXInt[L] = new __DFUIntFromXInt(left)
      sealed class __DFUIntFromLong[L <: Long](left: L)(implicit
          di: DummyImplicit
      ) extends AbleOps[L](left)
      final implicit def __DFUIntFromLong[L <: Long](left: L)(implicit
          di: DummyImplicit
      ): __DFUIntFromLong[L] = new __DFUIntFromLong(left)
      sealed class __DFUIntFromXLong[L <: XLong](left: L)(implicit
          di: DummyImplicit
      ) extends AbleOps[ValueOf[L]](new ValueOf(left))
      final implicit def __DFUIntFromXLong[L <: XLong](left: L)(implicit
          di: DummyImplicit
      ): __DFUIntFromXLong[L] = new __DFUIntFromXLong(left)
      sealed class __DFUIntFromBigInt[L <: BigInt](left: L)
          extends AbleOps[L](left)
      final implicit def __DFUIntFromBigInt[L <: BigInt](
          left: L
      ): __DFUIntFromBigInt[L] = new __DFUIntFromBigInt[L](left)
      final implicit def __ofDFUInt[W](left: DFUInt[W]): Able[DFUInt[W]] =
        new Able(left)
      final implicit class __ExtendableDFUIntOps[LW](
          val left: DFUInt[LW] with Extendable
      ) {
        def +[R](right: Exact[R])(implicit
            op: `Op+`.Builder[DFUInt[LW], true, R]
        ) = op(left, right)
        def -[R](right: Exact[R])(implicit
            op: `Op-`.Builder[DFUInt[LW], true, R]
        ) = op(left, right)
      ...
      final implicit class __DFUIntOps[LW](val left: DFUInt[LW]) {
        def +[R](right: Exact[R])(implicit
            op: `Op+`.Builder[DFUInt[LW], false, R]
        ) = op(left, right)
        def +^[R](right: Exact[R])(implicit
            op: `Op+^`.Builder[DFUInt[LW], true, R]
        ) = op(left, right)
        ...
        def <[R](right: Exact[R])(implicit op: `Op<`.Builder[DFUInt[LW], R]) =
          op(left, right)
        def >[R](right: Exact[R])(implicit op: `Op>`.Builder[DFUInt[LW], R]) =
          op(left, right)
        def <=[R](right: Exact[R])(implicit op: `Op<=`.Builder[DFUInt[LW], R]) =
  ...
  object `Op:=,<>` {
    ...
    trait Implicits {
      final implicit def __DFUInt_ac_DFUInt[LW, RW](implicit
          ctx: DFAny.Context,
      ...
      final implicit def __DFUInt_ac_DFBits[LW, RW](implicit
          ctx: DFAny.Context,
          checkLWvRW: `LW == RW`.CheckedShell[LW, RW]
      ...
      final implicit def __DFUInt_ac_DFBitsConst[LW, R, RW](implicit
          ctx: DFAny.Context,
  ...
  protected abstract class `Ops+Or-`[Op <: Func2.Op.Negateable](val op: Op) {
    ...
    object Builder {
      ...
      trait DetailedBuilder[L, LW, LE, R, RW] {
        type Out
        def apply(
            properLR: (L, R) => (Func2.Op.Negateable, DFUInt[LW], DFUInt[RW])
          ...
          new DetailedBuilder[L, LW, LE, R, RW] {
            ...
            def apply(
                properLR: (
                    L,
                    R
                ) => (Func2.Op.Negateable, DFUInt[LW], DFUInt[RW])
            ): Builder.Aux[L, LE, R, Out] =
              new Builder[L, LE, R] {
                type Out = DFUInt[OutW]
      ...
      implicit def evConst_op_DFUInt[L, LW, LE, RW](implicit
          ctx: DFAny.Context,
          lConst: DFAny.Const.AsIs.Aux[Type[RW], L, _ <: Type[LW]],
  ...
  protected abstract class `Op*`[Op <: Func2.Op.OptionalCarry](val op: Op) {
    ...
    object Builder {
      ...
      trait DetailedBuilder[L, LW, LE, R, RW] {
        type Out
        def apply(
            properLR: (L, R) => (DFUInt[LW], DFUInt[RW])
        ): Builder.Aux[L, LE, R, Out]
      }
      object DetailedBuilder {
        implicit def ev[L, LW, LE, R, RW, OutW](implicit
      ...
      implicit def evDFUInt_op_Const[LW, LE, R, RW](implicit
        ...
        detailedBuilder((left, rightNum) =>
          (left, rConst(left.dfType, rightNum).asInstanceOf[DFUInt[RW]])
        )
      ...
      implicit def evConst_op_DFUInt[L, LW, LE, RW](implicit
        ...
        detailedBuilder((leftNum, right) =>
          (lConst(right.dfType, leftNum).asInstanceOf[DFUInt[LW]], right)
        )
    }
  }
  ...
  object `Op<<` extends OpsShift[Type](Func2.Op.<<) {
    def tokenFunc[LW](left: DFUInt.Token, right: DFUInt.Token): DFUInt.Token =
      left << right
  }
  object `Op>>` extends OpsShift[Type](Func2.Op.>>) {
    def tokenFunc[LW](left: DFUInt.Token, right: DFUInt.Token): DFUInt.Token =
