...
sealed trait DFConditional[Owner <: DFConditional.Block] extends FSM.Capable {
  type TRet
  val owner: Owner
  private[DFiant] def applyBlock(block: => TRet)(implicit
      ctx: DFBlock.Context
  ): Unit
}
...
object DFConditional {
  sealed trait Of[Owner <: DFConditional.Block, Ret]
      extends DFConditional[Owner] { type TRet = Ret }
  sealed trait Block
      extends DFBlock
      with CanBeGuarded
  ...
  type PrevBlockRefOption[+O <: Block] =
    DFMember.OwnedRefOption[PrevBlockRefOption.Type, O]
  object PrevBlockRefOption {
    trait Type extends DFMember.OwnedRef.Type
    implicit val ev: Type = new Type {}
    def unapply(ref: DFMember.OwnedRef): Boolean =
      ref.refType match {
  ...
  final case class IfElseBlock(
      condRefOption: CondRefOption,
      prevBlockRefOption: PrevBlockRefOption[IfElseBlock],
    ...
    )(implicit getSet: MemberGetSet): DFMember =
      getSet.set(this)(m => m.copy(tags = tagsFunc(m.tags)))
  }
  object IfElseBlock {
    def apply(
        cond: Option[DFAny.Member],
        prevBlock: Option[IfElseBlock]
  ...
  final case class MatchHeader(
      matchValRef: MatchValRef,
      matchConfig: MatchConfig,
      ownerRef: DFOwner.Ref,
      tags: DFMember.Tags
  ) extends CanBeGuarded
      with DFAny.CanBeAnonymous {
    protected[DFiant] def =~(
        that: DFMember
    )(implicit getSet: MemberGetSet): Boolean =
    ...
    def codeString(implicit printer: CSPrinter): String = {
      import printer.config._
      matchConfig match {
        case MatchConfig.NoOverlappingCases =>
          s"$DF matchdf(${matchValRef.refCodeString})"
        case MatchConfig.AllowOverlappingCases =>
          s"$DF matchdf(${matchValRef.refCodeString}, MatchConfig.AllowOverlappingCases)"
      }
    }
    def setTags(
        tagsFunc: DFMember.Tags => DFMember.Tags
  ...
  object MatchHeader {
    ...
    def apply(matchVal: DFAny.Member, matchConfig: MatchConfig)(implicit
  ...
  final case class CaseBlock(
      matchHeaderRef: DFConditional.MatchHeader.Ref,
      prevBlockRefOption: PrevBlockRefOption[CaseBlock],
      patternOption: Option[DFAny.Pattern],
      ownerRef: DFOwner.Ref,
      tags: DFMember.Tags
  ) extends Block {
    protected[DFiant] def =~(
  ...
  object CaseBlock {
    def apply(
        ...
        prevBlockOption: Option[CaseBlock],
        patternOption: Option[DFAny.Pattern]
      ...
      implicit lazy val ret: CaseBlock with DFMember.RefOwner =
        ctx.db.addMemberOf[CaseBlock](
          CaseBlock(
            matchHeader,
            OwnedRefOption(prevBlockOption),
            patternOption,
            ctx.owner,
            ctx.meta
          )
  ...
  sealed abstract class WithRetVal[
      Owner <: DFConditional.Block, Type <: DFAny.Type
  ](
      val retVar: DFAny.VarOf[Type]
  ) extends DFConditional.Of[Owner, DFAny.Of[Type]]
      with DFAny.Of[Type] {
    val member: DFAny.Member = retVar

    private[DFiant] def applyBlock(block: => DFAny.Of[Type])(implicit
        ctx: DFBlock.Context
    ): Unit =
      ctx.db.OwnershipContext.injectOwnerAndRun(ctx.container, owner) {
  ...
  object WithRetVal {
    final class IfElseBlock[Type <: DFAny.Type, HasElse](
        retVar: DFAny.VarOf[Type],
        condOption: Option[DFBool],
        prevBlockOption: Option[DFConditional.IfElseBlock]
    )(block: => DFAny.Of[Type])(implicit ctx: DFBlock.Context)
        extends WithRetVal[DFConditional.IfElseBlock, Type](retVar) {
      val owner: DFConditional.IfElseBlock =
        DFConditional.IfElseBlock(condOption.map(_.member), prevBlockOption)
      applyBlock(block)
    }
    object IfElseBlock {
      def newIf[Type <: DFAny.Type, B, C](
          retVar: DFAny.VarOf[Type],
          cond: DFBool
      )(block: => DFAny.Of[Type])(implicit
          ctx: DFBlock.Context
      ): IfElseBlock[Type, true] =
        new IfElseBlock[Type, true](retVar, Some(cond), None)(block)

      implicit class IfElseBlockOps[Type <: DFAny.Type](
          notElse: IfElseBlock[Type, true]
      ) {
        import notElse.{retVar, dfType, owner}
        def elsedf[B](block: => Exact[B])(implicit
            ctx: DFBlock.Context,
            blockConv: retVar.Arg[B]
        ): IfElseBlock[Type, false] =
          new IfElseBlock[Type, false](retVar, None, Some(owner))(
            blockConv(dfType, block)
          )
        def elseifdf[C, B](cond: Exact[C])(block: => Exact[B])(implicit
            ctx: DFBlock.Context,
            condArg: DFBool.Arg[C],
            blockConv: retVar.Arg[B]
        ): IfElseBlock[Type, true] =
          new IfElseBlock[Type, true](retVar, Some(condArg(cond)), Some(owner))(
            blockConv(dfType, block)
          )
      }
    }
    ...
    final class MatchHeader[Type <: DFAny.Type, MVType <: DFAny.Type](
        val retVar: DFAny.VarOf[Type],
        matchVal: DFAny.Of[MVType],
        matchConfig: MatchConfig
    )(implicit ctx: DFMember.Context) {
      val dfType: Type   = retVar.dfType
      val mvType: MVType = matchVal.dfType
      val member: DFConditional.MatchHeader =
        DFConditional.MatchHeader(matchVal, matchConfig)
      def casedf[MC, B](pattern: mvType.TPatternAble[MC]*)(block: => Exact[B])(
          implicit
          ctx: DFBlock.Context,
          patternBld: mvType.TPatternBuilder[MVType],
          retBld: retVar.Arg[B]
      ): CaseBlock[Type, MVType, true] =
        new CaseBlock[Type, MVType, true](
          this,
          None,
          Some(patternBld(mvType, pattern))
        )(retBld(dfType, block))
    }

    final class CaseBlock[
        Type <: DFAny.Type,
        MVType <: DFAny.Type,
        HasMoreCases
    ](
        val matchHeader: MatchHeader[Type, MVType],
        prevBlockOption: Option[DFConditional.CaseBlock],
        patternOption: Option[DFAny.Pattern]
    )(block: => DFAny.Of[Type])(implicit ctx: DFBlock.Context)
        extends WithRetVal[DFConditional.CaseBlock, Type](matchHeader.retVar) {
      val mvType: MVType = matchHeader.mvType
      val owner: DFConditional.CaseBlock =
        DFConditional.CaseBlock(
          matchHeader.member,
          prevBlockOption,
          patternOption
        )
    ...
    object CaseBlock {
      implicit class CaseBlockOps[Type <: DFAny.Type, MVType <: DFAny.Type](
          notOthers: CaseBlock[Type, MVType, true]
      ) {
        import notOthers.{dfType, mvType, matchHeader, owner}
        def casedf[MC, B](
            pattern: mvType.TPatternAble[MC]*
        )(block: => Exact[B])(implicit
            ctx: DFBlock.Context,
            patternBld: mvType.TPatternBuilder[MVType],
            retBld: notOthers.retVar.Arg[B]
        ): CaseBlock[Type, MVType, true] =
          new CaseBlock[Type, MVType, true](
            matchHeader,
            Some(owner),
        ...
        def casedf_[MC, B](block: => Exact[B])(implicit
            ctx: DFBlock.Context,
            retBld: DFAny.`Op:=,<>`.Builder[Type, B]
        ): CaseBlock[Type, MVType, false] =
          new CaseBlock[Type, MVType, false](
            matchHeader,
            Some(owner),
            None
          )(retBld(dfType, block))
  ...
  sealed trait NoRetVal[Owner <: DFConditional.Block]
      extends DFConditional.Of[Owner, Unit] {
    private[DFiant] def applyBlock(block: => Unit)(implicit
        ctx: DFBlock.Context
    ): Unit =
      ctx.db.OwnershipContext.injectOwnerAndRun(ctx.container, owner)(block)
  }
  object NoRetVal {
    final class IfElseBlock[HasElse](
        condOption: Option[DFBool],
        prevBlockOption: Option[DFConditional.IfElseBlock]
    )(block: => Unit)(implicit ctx: DFBlock.Context)
        extends NoRetVal[DFConditional.IfElseBlock] {
      val owner: DFConditional.IfElseBlock =
        DFConditional.IfElseBlock(condOption.map(_.member), prevBlockOption)
      applyBlock(block)
    }
    object IfElseBlock {
      implicit class IfElseBlockOps(notElse: IfElseBlock[true]) {
        ...
        def elsedf[B](block: => Unit)(implicit
            ctx: DFBlock.Context
        ): IfElseBlock[false] = new IfElseBlock[false](None, Some(owner))(block)
        def elseifdf[C, B](cond: Exact[C])(block: => Unit)(implicit
            ctx: DFBlock.Context,
            condArg: DFBool.Arg[C]
        ): IfElseBlock[true] =
          new IfElseBlock[true](Some(condArg(cond)), Some(owner))(block)
      }
    }
    ...
    object HasCaseDF {
      implicit class CaseBlockOps[MVType <: DFAny.Type](
          notOthers: HasCaseDF[MVType, true]
      ) {
        ...
        def casedf[MC, B](pattern: mvType.TPatternAble[MC]*)(block: => Unit)(
            ...
            ctx: DFBlock.Context,
            patternBld: mvType.TPatternBuilder[MVType]
        ): CaseBlock[MVType, true] =
          new CaseBlock[MVType, true](
            matchHeader,
            thisBlockOption,
            Some(patternBld(mvType, pattern))
          )(block)
        def casedf_[MC, B](block: => Unit)(implicit
            ctx: DFBlock.Context
        ): CaseBlock[MVType, false] =
          new CaseBlock[MVType, false](
            matchHeader,
            thisBlockOption,
            None
          )(block)
    ...
    final class CaseBlock[MVType <: DFAny.Type, HasMoreCases](
        ...
        ctx: DFBlock.Context
    ) extends NoRetVal[DFConditional.CaseBlock]
        with HasCaseDF[MVType, HasMoreCases] {
      val mvType: MVType = matchHeader.mvType
      val owner: DFConditional.CaseBlock =
        DFConditional.CaseBlock(
          matchHeader.member,
      ...
      protected val thisBlockOption: Option[DFConditional.CaseBlock] = Some(
        owner
      )
      applyBlock(block)
    }
