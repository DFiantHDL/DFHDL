...
class MetaContextGenPhase(setting: Setting) extends CommonPhase:
  ...
  extension (tree: Tree)(using Context)
    // A context argument that is a synthetic inline proxy (e.g. `x$3$proxy1`)
    // generated by macro inlining. Such proxies already wrap a positioned context.
    // The reference is often wrapped in `Inlined`/`Typed` nodes (whose own symbol is
    // NoSymbol), so we strip the wrappers to reach the proxy reference and test its
    // `InlineProxy` flag.
    def isProxyContext: Boolean =
      tree match
        case Inlined(_, _, expansion) => expansion.isProxyContext
        case Typed(expr, _)           => expr.isProxyContext
        case _                        => tree.symbol.is(InlineProxy)
          ...
          treeOwnerApplyMap.get(origApply) match
            case Some(ownerTree, srcPos) =>
              getMetaInfo(ownerTree, srcPos) match
                case Some(metaInfo) =>
                  // When an anonymous (nameless) meta is stamped onto a synthetic proxy
                  // context (e.g. `x$N$proxyM`) produced by macro inlining, and its only
                  // position leaks outside the compilation unit (e.g. into the macro's own
                  // internals/Exact.scala), that position is bogus for user-facing reporting.
                  // We override it with the nearest enclosing user-code position when one
                  // exists. The `setMeta` itself must stay (it preserves value grouping), so
                  // if no better position is found we keep the original one unchanged.
                  val pos =
                    if (
                  ...
                  fixedApply.replaceArg(
                    argTree,
                    argTree.setMeta(metaInfo.nameOpt, pos, metaInfo.docOpt, metaInfo.annotations)
                  )
                case None =>
  ...
  // The nearest enclosing apply (currently on the stack) whose position lies in the
  // compilation unit being compiled. Used to recover a user-code position when a
  ...
  private def rejectBadPrimitiveOps(tree: Apply, pos: util.SrcPos)(using
    ...
    tree match
      ...
      case Apply(Select(lhs, fun), List(HackedGuard(_)))
