...
package object internals {
  ...
  def intToBigIntBits(value: Int): BigInt =
    BigInt(Integer.toHexString(value), 16)
  def longToBigIntBits(value: Long): BigInt = BigInt(LongtoHexString(value), 16)
  def floatToBigIntBits(value: Float): BigInt =
    intToBigIntBits(floatToRawIntBits(value))
  def doubleToBigIntBits(value: Double): BigInt =
    longToBigIntBits(doubleToRawLongBits(value))
  ...
  object BitsWidthOf {
    ...
    object SignedCfg {
      ...
      type Int[S, V] =
        TwoFace.Int.Shell2[CalcInt, S, scala.Boolean, V, scala.Int]
      type IntAux[S, V, Ret_Out] = Int[S, V] { type Out = Ret_Out }
      type Long[S, V] =
        TwoFace.Int.Shell2[CalcLong, S, scala.Boolean, V, scala.Long]
  ...
  def bigIntToBinaryString(value: BigInt, width: Int = 0): String = {
    val _width =
      if (width <= value.bitsWidth(false)) value.bitsWidth(false) else width
    if (value >= 0) {
  ...
  implicit class ReflectionClassExtras(extended: Any) {
    ...
    def getNestedDeclaredFieldsOf[T, B](
        subClass: Class[_],
        fieldApply: (Field, T) => B
    ): List[B] = {
      def allFieldsFrom(c: Class[_]): List[Field] = {
        if (c == null) List()
        else
          c.getDeclaredFields
            .filter(f => subClass.isAssignableFrom(f.getType))
            .toList ++ allFieldsFrom(c.getSuperclass)
      }
  ...
  object BitsHiLo extends Checked1Param.Int {
    type Cond[H, L] = H >= L
    type Msg[H, L] = "Low index " + ToString[
      L
  ...
  object BitIndex extends Checked1Param.Int {
    ...
    type Msg[I, W] =
      "Index " + ToString[I] + " is out of range of width/length " + ToString[W]
    type ParamFace = Int
  }

  object PartWidth extends Checked1Param.Int {
    type Cond[PW, W] = (PW <= W) && (PW > 0)
    type Msg[PW, W] = "Partial width bit selection " + ToString[
      PW
    ] + " is out of range of width " + ToString[W]
    type ParamFace = Int
  }

  object ExtWidth extends Checked1Param.Int {
    type Cond[EW, W] = EW > W
    type Msg[EW, W] = "Extended width " + ToString[
      EW
    ] + " is not larger than current width " + ToString[W]
    type ParamFace = Int
  }

  object AsWidth extends Checked1Param.Int {
    type Cond[AW, CW] = AW == CW
    type Msg[AW, CW] = "The aliased type width " + ToString[
      AW
    ] + " is not equal to the current width " + ToString[CW]
    type ParamFace = Int
  }
  ...
  object BitsWidth extends Checked0Param.Int {
    type Cond[W] = (W > 0)
    type Msg[W] =
      "DFBits/DFUInt width must be positive. Found width = " + ToString[W]
  }

  object SIntWidth extends Checked0Param.Int {
    type Cond[W] = (W > 1)
    type Msg[W] =
      "DFSInt width must be 2 or larger. Found width = " + ToString[W]
  }
  ...
  object Natural {
    type MsgCommon[N] =
      "Argument must be a natural number, but found " + ToString[N]
    ...
    object BigInt extends Checked1Param.Boolean {
      ...
      def unsafeCheck(
          n: BigInt
      )(implicit chk: BigInt.CheckedShell[Boolean, String]): Unit =
  ...
  object `LW >= RW` extends Checked1Param.Int {
    ...
    type Msg[LW, RW] =
      "This operation does not permit applying a wider RHS expression. Found: LHS-width = " + ToString[
        LW
      ] + " and RHS-width = " + ToString[RW]
    type ParamFace                     = Int
  ...
  object `LW == RW` extends Checked1Param.Int {
    type Cond[LW, RW] = LW == RW
    type Msg[LW, RW] =
      "This operation does not permit applying different width DF variables. Found: LHS-width = " + ToString[
        LW
      ] + " and RHS-width = " + ToString[RW]
    type ParamFace = Int
  }
  ...
  implicit class IterableStringsOps(list: Iterable[String]) {
    def longestCommmonPrefix: String =
      list.foldLeft("")((_, _) =>
        (list.min.view lazyZip list.max.view)
  ...
  implicit class GroupByOrderedImplicitImpl[T](val seq: Iterable[T])
      extends AnyVal {
  ...
  implicit class WhiteboxMacroExt(c: scala.reflect.macros.whitebox.Context) {
    ...
    private val defaultAnnotatedSym: Option[TypeSymbol] =
      if (c.enclosingImplicits.isEmpty) None
        ...
        c.enclosingImplicits.last.pt match {
