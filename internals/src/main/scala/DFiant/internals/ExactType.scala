...
trait ExactType[UniqueTag] {
  ...
  object Summon {
    type Aux[-From, V, +To, Out0 <: To] = Summon[From, V, To] {
      type Out = Out0
    }
    trait SAM[-From, V, To]
        extends ExactType.SummonGen[UniqueTag, From, V, To] { type Out = To }
  }
...
object ExactType {
  @implicitNotFound(
    "Unsupported value type ${V} (supported values are defined by ${From})"
  )
  ...
  object ConvGen {
    implicit def conv[UniqueTag, From, To, Out <: To](
        value: Any
    ): ConvGen[UniqueTag, From, To] =
      macro convMacro[UniqueTag, From, To, Out]
    def convMacro[UniqueTag, From, To, Out <: To](
        c: whitebox.Context
    )(value: c.Tree)(implicit
