...
object Meta {
  ...
  //Helper defs
  /////////////////////////////////////////////////////////
  private def getOwnerName(c: blackbox.Context)(owner: c.Symbol): String =
    owner.name.decodedName.toString.trim
  @tailrec private def _getValidOwner(
      c: blackbox.Context
  )(owner: c.Symbol): c.Symbol = {
    val name = getOwnerName(c)(owner)
    if (
      name == "<init>" || (name
        .startsWith("<local ") && name.endsWith(">")) || name.contains("$")
    ) _getValidOwner(c)(owner.owner)
    else owner
  }
  private def getValidOwner(c: blackbox.Context) =
    _getValidOwner(c)(c.internal.enclosingOwner)

  @tailrec private def _isOwnedByAnonymousClass(
      c: blackbox.Context
  )(owner: c.Symbol): Boolean = {
    if (owner.isClass && owner.name.toString != "$anonfun")
      owner.name.toString.contains("$")
    else if (owner.isConstructor)
      _isOwnedByAnonymousClass(c)(
        owner.owner.owner
      ) //jumping above the current class that belongs to this constructor
    else if (owner.isPackage) false
    else _isOwnedByAnonymousClass(c)(owner.owner)
  }
  private def isOwnedByAnonymousClass(c: blackbox.Context) =
    _isOwnedByAnonymousClass(c)(c.internal.enclosingOwner)
  /////////////////////////////////////////////////////////
  ...
  final case class Position(file: String, line: Int, column: Int) {
    def >(that: Position): Boolean = {
      assert(
        file == that.file,
        "Can only compare positions within the same file"
      )
...
//    }
    ...
    object OfType {
      ...
      def evMacro[T](
          c: blackbox.Context
      )(implicit t: c.WeakTypeTag[T]): c.Expr[OfType[T]] = {
        import c.universe._
  ...
  object SymbolOf {
    ...
    def evMacro[T](
        c: blackbox.Context
    )(implicit t: c.WeakTypeTag[T]): c.Expr[SymbolOf[T]] = {
      import c.universe._
      val sym  = symbolOf[T]
  ...
  implicit def ev(implicit
      lateConstructionConfig: LateConstructionConfig
  ): Meta = macro evMacro
  def evMacro(
      c: blackbox.Context
  )(lateConstructionConfig: c.Tree): c.Expr[Meta] = {
    import c.universe._
...
//    println(name, anonymous, owner.isMethod, c.enclosingPosition)
  ...
  object IsVar {
    ...
    def evMacro(c: blackbox.Context): c.Expr[IsVar] = {
      import c.universe._
      val owner = getValidOwner(c)
      if (owner.isTerm && owner.asTerm.isVar)
        c.Expr[IsVar](q"""${c.prefix}()""")
      else c.abort(c.enclosingPosition, "var is not allowed")
    }
  }

  type ForceNotVar[Sym] =
    RequireMsgSym[![ImplicitFound[IsVar]], VarDFTypes.Msg, Sym]
  final object VarDFTypes
      extends ErrorMsg(
