...
class ExactTypeSpec extends munit.FunSuite {
  ...
  def foo(positive : Positive.Conv[None.type, Positive[_ <: Int]]) : positive.Out = positive(None)

  implicit def positiveSummon[T <: XInt](
    implicit req : RequireMsg[T > 0, "Must be positive. Got " + ToString[T] + " instead"]
  ...
  test("Value should return proper error") {
    assertNoDiff(compileErrors("""foo(-1)"""), """|error: Must be positive. Got -1 instead
                                                  |foo(-1)
                                                  |    ^
                                                  |""".stripMargin)
  }
  ...
  def fooVarArgs(even : Positive.Conv[None.type, Int]*) : Seq[Int] = even.map(_.apply(None))
  ...
  test("VarArgs Value should return proper error") {
    assertNoDiff(compileErrors("""fooVarArgs(2,4,5,8)"""), """|error: Must be even. Got 5 instead
                                                              |fooVarArgs(2,4,5,8)
                                                              |               ^
                                                              |""".stripMargin)
  }
