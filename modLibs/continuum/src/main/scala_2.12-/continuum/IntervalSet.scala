...
object IntervalSet extends {
  ...
  implicit def canBuildFrom[T](implicit
    ...
    new CanBuildFrom[IntervalSet[_], Interval[T], IntervalSet[T]] {
      def apply(
          from: IntervalSet[_]
      ): mutable.Builder[Interval[T], IntervalSet[T]]           = newBuilder[T]
...
class IntervalSet[T](tree: RB.Tree[Interval[T], Unit])(implicit
  ...
  override def from(from: Interval[T]): IntervalSet[T] =
    newSet(RB.from(tree, from))
  override def to(to: Interval[T]): IntervalSet[T] = newSet(RB.to(tree, to))
  override def until(until: Interval[T]): IntervalSet[T] =
  ...
  def intersects(interval: Interval[T]): Boolean =
    from(interval).head intersects interval
