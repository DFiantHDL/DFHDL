...
trait Generators {
  ...
  implicit def arbOpenBound[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[Open[T]] =
    Arbitrary { for (cut <- arbitrary[T]) yield Open(cut) }

  implicit def arbClosedBound[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[Closed[T]] =
    Arbitrary { for (cut <- arbitrary[T]) yield Closed(cut) }

  implicit def arbUnbounded[T](implicit
      conv: T => Ordered[T]
  ): Arbitrary[Unbounded[T]] = Arbitrary(Unbounded[T]())

  implicit def arbBound[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[Bound[T]] =
    Arbitrary(
      Gen.frequency(
  ...
  implicit def arbLesserRay[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[LesserRay[T]] =
    Arbitrary(for (b <- arbitrary[Bound[T]]) yield LesserRay(b))

  implicit def arbGreaterRay[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[GreaterRay[T]] =
    Arbitrary(for (b <- arbitrary[Bound[T]]) yield GreaterRay(b))

  implicit def arbRay[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[Ray[T]] =
    Arbitrary(Gen.oneOf(arbitrary[GreaterRay[T]], arbitrary[LesserRay[T]]))

  implicit def arbInterval[T: Arbitrary](implicit
      conv: T => Ordered[T]
    ...
    Arbitrary {
      ...
      def interval(a: Bound[T], b: Bound[T]) =
        if (Interval.validate(GreaterRay(a), LesserRay(b)))
          new Interval(GreaterRay(a), LesserRay(b))
  ...
  implicit def arbIntInterval: Arbitrary[Interval[Int]] =
    Arbitrary {
      def genOpenAbove(below: Bound[Int]): Gen[Bound[Int]] =
        below match {
          case Closed(l) if l == Int.MaxValue => arbitrary[Unbounded[Int]]
          case Open(l) if l == Int.MaxValue   => arbitrary[Unbounded[Int]]
          case Closed(l) =>
            for (n <- Gen.choose(l + 1, Int.MaxValue)) yield Open(n)
          case Open(l) =>
            for (n <- Gen.choose(l + 1, Int.MaxValue)) yield Open(n)
          case Unbounded() => arbitrary[Open[Int]]
        }

      def genClosedAbove(below: Bound[Int]): Gen[Bound[Int]] =
        below match {
          case Open(l) if l == Int.MaxValue => arbitrary[Unbounded[Int]]
          case Closed(l) =>
            for (n <- Gen.choose(l, Int.MaxValue)) yield Closed(n)
          case Open(l) =>
            for (n <- Gen.choose(l + 1, Int.MaxValue)) yield Closed(n)
          case Unbounded() => arbitrary[Closed[Int]]
        }

      def genLesserRay(g: GreaterRay[Int]): Gen[LesserRay[Int]] =
        Gen
          .frequency(
  ...
  implicit def shrinkGreaterRay[T: Shrink](implicit
      conv: T => Ordered[T]
  ): Shrink[GreaterRay[T]] =
    Shrink { ray =>
      ray.bound match {
        case Closed(n) =>
          for (np <- Shrink.shrink(n)) yield GreaterRay(Closed(np))
        case Open(n)     => for (np <- Shrink.shrink(n)) yield GreaterRay(Open(np))
        case Unbounded() => Stream.empty
      }
    }

  implicit def shrinkLesserRay[T: Shrink](implicit
      conv: T => Ordered[T]
  ): Shrink[LesserRay[T]] =
    Shrink { ray =>
      ray.bound match {
        case Closed(n) =>
          for (np <- Shrink.shrink(n)) yield LesserRay(Closed(np))
        case Open(n)     => for (np <- Shrink.shrink(n)) yield LesserRay(Open(np))
        case Unbounded() => Stream.empty
      }
    }

  implicit def shrinkInterval[T: Shrink](implicit
      conv: T => Ordered[T]
  ): Shrink[Interval[T]] =
    Shrink { interval =>
      for {
        lower <- Shrink.shrink(interval.lower)
        upper <- Shrink.shrink(interval.upper)
        if Interval.validate(lower, upper)
      } yield new Interval(lower, upper)
    }
  ...
  def genIntervalRange: Gen[Interval[Int]] = {
    ...
    def genClosed: Gen[Interval[Int]] =
      Gen.sized { size =>
        for {
          lower <- arbitrary[Int] if lower + size >= lower
    ...
    def genOpenClosed: Gen[Interval[Int]] =
      Gen.sized { size =>
        for {
          lower <- arbitrary[Int] if lower + size > lower
    ...
    def genClosedOpen: Gen[Interval[Int]] =
      Gen.sized { size =>
        for {
          lower <- arbitrary[Int] if lower + size > lower
  ...
  implicit def arbIntervalSet[T: Arbitrary](implicit
      conv: T => Ordered[T]
  ): Arbitrary[IntervalSet[T]] =
    Arbitrary(
      for (intervals <- arbitrary[Array[Interval[T]]])
        yield IntervalSet(intervals: _*)
    )
  ...
  implicit def arbIntIntervalSet: Arbitrary[IntervalSet[Int]] =
    Arbitrary(
      for (intervals <- arbitrary[Array[Interval[Int]]])
        yield IntervalSet(intervals: _*)
    )
}
