 ...
 * @tparam T type of values contained in the continuous, infinite, total-ordered set which the
  *           ray operates on.
  */
sealed abstract class Ray[T](implicit conv: T => Ordered[T])
    extends (T => Boolean) {
...
case class GreaterRay[T](bound: Bound[T])(implicit conv: T => Ordered[T])
    extends Ray[T]
    with Ordered[GreaterRay[T]] {
  ...
  override def tangent: Option[LesserRay[T]] =
    bound match {
  ...
  override def isSameDirection(other: Ray[T]): Boolean =
    other match {
...
case class LesserRay[T](bound: Bound[T])(implicit conv: T => Ordered[T])
    extends Ray[T]
    with Ordered[LesserRay[T]] {
  ...
  override def tangent: Option[GreaterRay[T]] =
    bound match {
