...
package bound {
  ...
  case class Closed[T](value: T)(implicit conv: T => Ordered[T])
      extends Bound[T] {
    override def isAbove(otherBound: Bound[T]): Boolean =
      otherBound match {
    ...
    override def isBelow(otherBound: Bound[T]): Boolean =
      otherBound match {
        case Open(otherC)   => value <= otherC
        case Closed(otherC) => value <= otherC
        case Unbounded()    => false
  ...
  case class Open[T](value: T)(implicit conv: T => Ordered[T])
      extends Bound[T] {
    override def isAbove(otherBound: Bound[T]): Boolean =
      otherBound match {
        case Open(otherC)   => value >= otherC
