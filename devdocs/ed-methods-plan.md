...
## OPEN ISSUES — the single authoritative list (2026-07-11)
...
1. **Differing applied `<> CONST` args across call sites** (demo-verified 2026-07-11 with
   ...
   (c) under default `@hw.pure`, the DFHDL re-emitter LOSES the second call's applied value
   (`z <> addK(a)` — missing `k = 7`, not even roundtrip-compilable): on a pure-cache hit
   the plugin-generated `genContainerParam` ValDefs (which live INSIDE the def body) never
   run, so the duplicate design context holds no `DesignParam` members and
   `Design.Inst.collectParamEntries` returns Nil → empty inst paramMap. With
   `@hw.pure(false)` both calls print their values correctly — (c) is cache-path-specific.
   Note (c) also means phantom const params have empty paramMap entries on cache hits —
   observably harmless today only because all calls within a design instance bind the same
   captured constant and printers hide phantom params anyway.
   Fix for (a)/(b): monomorphization (mangled per-value copies, the S5 plan) or, as a
   stopgap, an elaboration/stage check that rejects differing applied values per owning
   design.
   **Fix for (c) — PROPOSED MECHANISM (2026-07-11, "params ride the call"): applied
   parameters must never be discovered by running the body** (the general `@hw.pure`
   hazard: today `genContainerParam` ValDefs run inside the memoized body, and the pure
   cache key `(position, input DFTypes)` sees const args not at all). Make params fully
   symmetric with input ports:
   1. the plugin passes complete param specs `(name, appliedVal, defaultOpt, meta,
      phantom)` in the `designFromDef` call (it has all of this statically);
   2. body refs rewrite to `designFromDefGetParam(idx)` (mirror of
      `designFromDefGetInput`; runtime: `current.defParams` next to `defInputs`);
   3. `designFromDefImpl` creates the `DesignParam`s BEFORE `runFuncWithInputs` (and
      before the ports — dependent port types then resolve against existing params);
   4. `Design.Inst` builds paramMap directly from the spec list — `collectParamEntries`'s
      member scan + `unreachableNamedValues` recovery die for defs (failure mode removed,
      not patched);
   5. retire runtime auto-params for DF design defs too: extend the ED capture lift to DF
      defs (untagged/visible params) — runtime reachability stays only for design classes
      (bodies always run), migratable later via `MetaContextPlacerPhase`;
   6. cache key gains param DFTypes (TYPES only — value-parametric sharing is correct;
      per-value copies are a printing-stage concern, items 1a/1b/2).
   Rejected: replay recipes (persisted runtime discovery — can't re-evaluate captures in
   the new caller's context); applied VALUES in the cache key (wrong for DF parametric
   modules; conflates elaboration caching with HDL emission limits). Not addressed (item
   5's class): applied values leaking into Scala-level control flow.
   Design classes (verified 2026-07-11): `@hw.pure` memoization does NOT apply to design
   classes — `runFuncWithInputs` is reached only from `designFromDefImpl`; a class-level
   `@hw.pure` is inert. Classes always elaborate fully and get post-hoc structural dedup
   in `endDesign` (`sameDesignAs` → duplicates keep public members only) — always sound,
   just not elaboration-time-saving. Future class-level pure memoization would need the
   same call-side param carriage (ctor args are statically known at the instantiation
   site) PLUS cloning the cached first elaboration's public-member shape (ports are body
   members, unlike def inputs); cache key = (class position, param DFTypes).
...
5. **Plain-Scala captures silently break `@hw.pure` memoization** (§S8): a non-DFHDL field
   (`Int`, `Boolean`) read in a body is an invisible closure capture; two instances differing
   only in that field share one memoized body. Default-pure makes this default-on (worse than
   DF design defs, which are opt-in pure). UNDECIDED mitigation: plugin warning on non-DFHDL
   free refs of non-singleton types vs docs-only.
   Same hazard class — **`toScalaXYZ` extraction** (2026-07-11): a body extracting a Scala
   value from a DFHDL constant is impure by definition once that constant can vary per call
   (the extracted value steers Scala-level elaboration; structure stops being parametric).
   Future detection sketch: (a) static plugin scan for `toScalaXYZ` in bodies — cheap,
   incomplete (helper-def hole); (b) runtime flag — the conversions run under a DFC, so mark
   the design context "performed value extraction" and have `runFuncWithInputs` DECLINE to
   store the cache entry for such bodies (complete by construction; hits are unaffected
   since a stored body never extracted). Refinement: poison only extractions whose source
   depends on a def input/param (global-constant extraction is harmless).
