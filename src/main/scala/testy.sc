import DFiant._
import DFiant.internals._
//object Testy {
//  type BV[W] = BitVector//[W]
//  class Instr32(val instr : BV[32]) {
//    override def toString: String = instr.toShortString
//
//    override def equals(o: Any): Boolean = {
//      println("sfkhdfkghfdg")
//      instr == o.asInstanceOf[Instr32].instr
//    }
//  }
//  class RType(funct7 : BV[7], rs2 : BV[5], rs1 : BV[5], funct3 : BV[3], rd : BV[5], opcode : BV[7]) extends Instr32(funct7 ++ rs2 ++ rs1 ++ funct3 ++ rd ++ opcode)
//  object RType {
//    def unapply(isr : Instr32) : Option[(BV[7], BV[5], BV[5], BV[3], BV[5], BV[7])] = {
//      val instr = isr.instr
//      println("heh")
//      Some(instr.bits(31, 25), instr.bits(24, 20), instr.bits(19, 15), instr.bits(14, 12), instr.bits(11, 7), instr.bits(6, 0))
//    }
//  }
//
//  //    class InstrCO(funct7Check : BV[7], funct3Check : BV[3], opcodeCheck : BV[7]) {
//  //      def unapply(instr : Instr32) = instr match {
//  //        case RType(funct7,rs2,rs1,funct3,rd,opcode) if (funct7 == funct7Check) && (funct3 == funct3Check) && (opcode == opcodeCheck) => Some(rd, rs1, rs2)
//  //        case _ => None
//  //      }
//  //    }
//  //                                                                       funct7                funct3       opcode
//  case class SLLI(rd : BV[5], rs1 : BV[5], shamt : BV[5]) extends RType(b"0000000", shamt, rs1, b"001", rd, b"0010011")
//  case class SRLI(rd : BV[5], rs1 : BV[5], shamt : BV[5]) extends RType(b"0000000", shamt, rs1, b"101", rd, b"0010011")
//  case class SRAI(rd : BV[5], rs1 : BV[5], shamt : BV[5]) extends RType(b"0100000", shamt, rs1, b"101", rd, b"0010011")
//
//  //                                                                       funct7                funct3       opcode
//  case class ADD (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"000", rd, b"0110011")
//  case class SUB (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0100000", rs2,   rs1, b"000", rd, b"0110011")
//  case class SLL (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"001", rd, b"0110011")
//  case class SLT (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"010", rd, b"0110011")
//  case class SLTU(rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"011", rd, b"0110011")
//  case class XOR (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"100", rd, b"0110011")
//  case class SRL (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"101", rd, b"0110011")
//  case class SRA (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0100000", rs2,   rs1, b"101", rd, b"0110011")
//  case class OR  (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"110", rd, b"0110011")
//  case class AND (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2,   rs1, b"111", rd, b"0110011")
//
//  def decode(instr : Instr32) : Unit = {
//    instr match {
//      case SLLI(rd, rs1, shamt) =>
//      case SRLI(rd, rs1, shamt) =>
//      case SRAI(rd, rs1, shamt) =>
//
//      case ADD (rd, rs1, rs2)   => println("meme")
//      case SUB (rd, rs1, rs2)   =>
//      case SLL (rd, rs1, rs2)   =>
//      case SLT (rd, rs1, rs2)   =>
//      case SLTU(rd, rs1, rs2)   =>
//      case XOR (rd, rs1, rs2)   =>
//      case SRL (rd, rs1, rs2)   =>
//      case SRA (rd, rs1, rs2)   =>
//      case OR  (rd, rs1, rs2)   =>
//      case AND (rd, rs1, rs2)   =>
//      case _ =>
//        println("shit")
//    }
//  }
//}

object Testy {
  type BV[W] = BitVector//[W]
  class RType(funct7 : BV[7], rs2 : BV[5], rs1 : BV[5], funct3 : BV[3], rd : BV[5], opcode : BV[7])
  object RType {
    def unapply(instr : BV[32]) = Some(instr.bits(31, 25), instr.bits(24, 20), instr.bits(19, 15), instr.bits(14, 12), instr.bits(11, 7), instr.bits(6, 0))
    class InstrCO(funct7Check : BV[7], funct3Check : BV[3], opcodeCheck : BV[7]) {
      def unapply(instr : BV[32]) = instr match {
        case RType(funct7,rs2,rs1,funct3,rd,opcode) if (funct7 == funct7Check) && (funct3 == funct3Check) && (opcode == opcodeCheck) => Some(rd, rs1, rs2)
        case _ => None
      }
    }
  }
  class IType(imm : BV[12], rs1 : BV[5], funct3 : BV[3], rd : BV[5], opcode : BV[7])
  object IType {
    def unapply(instr : BV[32]) = Some(instr.bits(31, 20), instr.bits(19, 15), instr.bits(14, 12), instr.bits(11, 7), instr.bits(6, 0))
    class InstrCO(funct7Check : BV[7], funct3Check : BV[3], opcodeCheck : BV[7]) {
      def unapply(instr : BV[32]) = instr match {
        case IType(imm,rs1,funct3,rd,opcode) if (funct3 == funct3Check) && (opcode == opcodeCheck) => Some(rd, rs1, imm)
        case _ => None
      }
    }
  }

  //            RD         RS1          Shamt                                              Funct7   Funct3   Opcode
  class SLLI(rd : BV[5], rs1 : BV[5], shamt : BV[5]); object SLLI  extends RType.InstrCO(b"0000000", b"001", b"0010011")
  class SRLI(rd : BV[5], rs1 : BV[5], shamt : BV[5]); object SRLI  extends RType.InstrCO(b"0000000", b"101", b"0010011")
  class SRAI(rd : BV[5], rs1 : BV[5], shamt : BV[5]); object SRAI  extends RType.InstrCO(b"0100000", b"101", b"0010011")

  //            RD         RS1          RS2                                                Funct7   Funct3   Opcode
  class ADD (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object ADD  extends RType.InstrCO(b"0000000", b"000", b"0110011")
  class SUB (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SUB  extends RType.InstrCO(b"0100000", b"000", b"0110011")
  class SLL (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SLL  extends RType.InstrCO(b"0000000", b"001", b"0110011")
  class SLT (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SLT  extends RType.InstrCO(b"0000000", b"010", b"0110011")
  class SLTU(rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SLTU extends RType.InstrCO(b"0000000", b"011", b"0110011")
  class XOR (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object XOR  extends RType.InstrCO(b"0000000", b"100", b"0110011")
  class SRL (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SRL  extends RType.InstrCO(b"0000000", b"101", b"0110011")
  class SRA (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object SRA  extends RType.InstrCO(b"0100000", b"101", b"0110011")
  class OR  (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object OR   extends RType.InstrCO(b"0000000", b"110", b"0110011")
  class AND (rd : BV[5], rs1 : BV[5], rs2   : BV[5]); object AND  extends RType.InstrCO(b"0000000", b"111", b"0110011")

  //  case class ADD2 (rd : BV[5], rs1 : BV[5], rs2   : BV[5]) extends RType(b"0000000", rs2, rs1, b"000", rd, b"0110011") {
  //  }
  //  object ADD2 {
  //    def unapply(arg: BV[32]): Option[(BV[5], BV[5], BV[5])] = ???
  //
  //  }

  def decode(instr : BV[32]) : Unit = {
    instr match {
      case SLLI (rd, rs1, shamt) =>
      case SRLI (rd, rs1, shamt) =>
      case SRAI (rd, rs1, shamt) =>

      case ADD (rd, rs1, rs2) => println("heh")
      case SUB (rd, rs1, rs2) =>
      case SLL (rd, rs1, rs2) =>
      case SLT (rd, rs1, rs2) =>
      case SLTU(rd, rs1, rs2) =>
      case XOR (rd, rs1, rs2) =>
      case SRL (rd, rs1, rs2) =>
      case SRA (rd, rs1, rs2) =>
      case OR  (rd, rs1, rs2) =>
      case AND (rd, rs1, rs2) =>
      case _ =>
        println("shit")
    }
  }
}



val ins = b"00000000000000000000000000110011"
ins match {
  case Testy.RType(_,_,_,_,_,opcode) => println(opcode.toShortString)
}
//Testy.ADD(b"00000",b"00000",b"00000")
Testy.decode(ins)