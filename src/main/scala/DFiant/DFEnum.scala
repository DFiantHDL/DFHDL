...
//    def apply(that : Init.Able[DFEnum]*)(implicit dsn : DFDesign, op : Init.Builder[DFEnum], w : SafeInt[EntryWidth]): Var = newVar()
    ...
    object Token {
      ...
      def apply(value : Enum.Entry)(implicit w : SafeInt[EntryWidth]) : Token = new Token(Some(value.asInstanceOf[Entry]))
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    object Init extends Init {
      ...
      object Able {
        ...
        def toTokenSeq(width : Int, right : Seq[Able[DFEnum]])(implicit w : SafeInt[EntryWidth]) : Seq[Token] =
          right.toSeqAny.map(e => e match {
            case (t : Bubble) => Token(t)
            case (t : Enum.Entry) => Token(t)
            case (t : Token) => t
          })
      }
      trait Builder[L <: DFAny] extends DFAny.Init.Builder[L, Able]
      object Builder {
        implicit def ev(implicit dsn : DFDesign, w : SafeInt[EntryWidth]) : Builder[DFEnum] = (left, right) =>
