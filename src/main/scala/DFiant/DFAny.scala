...
//      println(s"initLB $fullName")
  ...
  case class Connector(toPort : DFAny, fromVal : DFAny)(implicit ctx0 : Connector.Context) extends DFAnyMember {
    ...
    def codeString : String = if (toPort.owner.isInstanceOf[Func2Comp[_,_,_]] && toPort.owner.asInstanceOf[Func2Comp[_,_,_]].isFolded) "" else connectCodeString
    final val id = getID
  }
  object Connector {
...
//         : TVal <> Dir = port(this.asInstanceOf[TVal], dir)
    ...
    def select[T, E](cond : DFBool)(thenSel : protComp.Op.Able[E], elseSel : protComp.Op.Able[T]) : TVal = ???
    def select[SW, T](sel : DFUInt[SW], default : => Option[TVal] = None)(args : protComp.Op.Able[T]*) : TVal = ???
    final val id = getID
  }
  object NewVar {
