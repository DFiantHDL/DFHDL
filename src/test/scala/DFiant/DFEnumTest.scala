...
class DFEnumAutoTest extends Properties("DFEnumAutoTest") {
  class Foo extends Enum.Auto(Enum.Encoding.Default) {
...
//    illTyped("""NotSealedDFEnum[Foo]()""", """No enumeration entries found or the Entry is not a sealed trait""")
//    illTyped("""NoEntriesDFEnum[Foo]()""", """No enumeration entries found or the Entry is not a sealed trait""")
  }
}
