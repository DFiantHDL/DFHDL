...
protected trait VerilogValPrinter extends AbstractValPrinter:
  ...
  // WHOLE-VECTOR init that the dialect cannot inline either is not lowered here:
  // `DropWholeVecAssign` turns it into a real `initial` block in the IR, so one arriving at this
  // printer is a shape that stage declined and that this dialect has no legal form for.
  def csDFValDclInitialBlock(dfVal: Dcl): String =
    val contents = dfVal.initRefList match
      case initRef :: Nil => s"${dfVal.getName} = ${initRef.refCodeString};"
