...
/**
  ...
  */
@df final class OmpssARR[AW] private (
    addrWidth: TwoFace.Int[AW],
    val factor: Int
)(
    dir: Either[PortDir, INOUT.type]
) extends DFInterface {
  val partitions =
    Vector.tabulate(factor)(new OmpssARR.Partition(addrWidth, factor, _)(dir))

  /**
    * Disabling all partitions read/write.
    * @param dontCareAddress when true the address is assigned with a bubble (don't care)
    */
  @df def disable(dontCareAddress: Boolean = true): Unit =
...
object OmpssARR {
  protected class PartitionName(idx: Int) extends DFOwner.NameFlatten {
    def apply(memberName: String, ownerName: String): String =
      s"${idx}_${memberName}0"
  }

  /**
    * An Array Partition Interface
    */
  @df final class Partition[AW] private[ompss] (
      addrWidth: TwoFace.Int[AW],
      val factor: Int,
      val idx: Int
  )(
  ...
  /**
    ...
    */
  final class ConfigNode[AW] private[OmpssARR] (
      addrWidth: TwoFace.Int[AW],
      factor: Int
  ) {
    def <>(
        dir: PortDir
    )(implicit ctx: ContextOf[OmpssARR[Nothing]]): OmpssARR[AW] =
  ...
  /**
    ...
    */
  def apply[S, F](
      size: Positive.Checked[S],
      factor: Positive.Checked[F]
  )(implicit
      addrWidth: BitsWidthOf.Int[(S / F) - 1]
  ): ConfigNode[addrWidth.Out] =
    new ConfigNode(addrWidth(size / factor - 1), factor)
  protected object BlockFlatten extends DFOwner.NameFlatten {
    def apply(memberName: String, ownerName: String): String =
      if (memberName == "offset")
        s"${ownerName}_$memberName" //the offset is special cases because there is no prefix here
      else
        s"m_axi_${ownerName}_$memberName" //Vivado adds "m_axi_" to the AXI interface signals
  }
}
