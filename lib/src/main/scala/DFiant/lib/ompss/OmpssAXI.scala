...
/**
  ...
  */
@df final class OmpssAXI private (
    dir: Either[PortDir, INOUT.type],
    flipped: Boolean
) extends DFInterface(OmpssAXI.PrefixNameFlatten) {
  ...
  final val axi =
    axiNode <> axiDir setNameFlatten (DFOwner.NameFlatten.IgnoreOwnerName)
...
object OmpssAXI {
  ...
  /**
    ...
    * @return the constructed interface
    */
  def <>(dir: INOUT.type)(implicit ctx: ContextOf[OmpssAXI]): OmpssAXI =
    new OmpssAXI(Right(dir), false)
