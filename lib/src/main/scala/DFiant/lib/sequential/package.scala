...
package object sequential {
  ...
  @df def doWhile[C](
      cond: => Exact[C]
  )(block: => Unit)(implicit arg: DFBool.Arg[C]): FSM =
    FSM {
      ifdf(arg(cond)) {
        block
      }.elsedf {
  ...
  @df private def privDoFor(range: Range, guard: Option[DFBool] = None)(
      block: DFUInt[Int] => Unit
  ): FSM = {
    assert(
      range.head >= 0 && range.last >= 0,
  ...
  @df def doFor[G](range: Range, guard: Exact[G])(
      block: DFUInt[Int] => Unit
  )(implicit arg: DFBool.Arg[G]): FSM =
