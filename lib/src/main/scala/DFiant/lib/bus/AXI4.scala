...
/**
  ...
  */
@df final class AXI4 private (axiDir: AXI4.Dir)(config: AXI4.Config)
    extends DFInterface {
...
object AXI4 {
  protected final class ConfigNode(config: Config) {
    ...
    /**
      ...
      */
    def <>(axiDir: Dir)(implicit ctx: ContextOf[AXI4]): AXI4 =
      new AXI4(axiDir)(config)
  }
  ...
  /**
    ...
    */
  final val SRO = AXI4(
    Config(rdEnabled = true, wrEnabled = false, simple = true)
  )
  ...
  /**
    ...
    */
  final val SWO = AXI4(
    Config(rdEnabled = false, wrEnabled = true, simple = true)
  )
  ...
  /**
    ...
    */
  final val SRW = AXI4(
    Config(rdEnabled = true, wrEnabled = true, simple = true)
  )
  ...
  object NoSuffixCapitalize extends NameFlatten {
    def apply(memberName: String, ownerName: String): String =
      s"${ownerName}${memberName.toUpperCase}"
  }

  @df abstract class Interface(streamDir: StreamDir)
      extends RVStream(streamDir, NoSuffixCapitalize)
