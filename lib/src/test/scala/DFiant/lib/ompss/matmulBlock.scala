...
@df class matmulBlock extends DFDesign {
  ...
  import io._
  final val cnt16         = DFUInt.until(BSIZE) init 0
  final val cnt8          = cnt16.resize(3)
  final val cnt4          = cnt8.resize(2)
  final val cnt32x8       = DFUInt.until(32) init 0
  final val cnt16xcnt32x8 = DFUInt.until(BSIZE) init 0
  final val aData         = DFBits(32)     := ?
  final val bData         = DFBits(32)     := ?
  final val cData         = DFBits(32)     := ?
  final val dataValid     = DFBit() init 0 := 0
  final val cWriteDone    = DFBit() init 0 := 0
...
object matmulBlock {
  ...
  object Matrix {
    ...
    def genIdentity: Matrix =
      Vector.tabulate(BSIZE)(i =>
        Vector.tabulate(BSIZE)(j => if (i == j) 1 else 0)
      )
    def genHexCount: Matrix =
      Vector.tabulate(BSIZE)(i => Vector.tabulate(BSIZE)(j => j * 16 + i))
  }
...
@df class matmulBlockDriver extends DFSimDesign {
  ...
  import io._
  import lib.sequential._
  private val resultOK = DFBool() := true
  ...
  private val aDebug = a.partitions.map { p =>
    val aMemArr = DFBits(32).X(64) init aMatrix.getPartition(p.factor, p.idx)
    aMemArr := aMemArr.prev
    val q = DFBits(32) init b0s
    p.q := q.prev
    ifdf(p.ce) {
  ...
  ifdf(aDebug.head._2.ce) {
    sim.report(
      msg"    $cycle    rd    a[${aDebug.head._2.address.uint}]    ${aDebug(
        0
      )._1} ${aDebug(1)._1} ${aDebug(2)._1} ${aDebug(3)._1}"
    )
  }

  private val bDebug = b.partitions.map { p =>
    val bMemArr = DFBits(32).X(64) init bMatrix.getPartition(p.factor, p.idx)
  ...
  ifdf(bDebug.head._2.ce) {
    sim.report(
      msg"    $cycle    rd    b[${bDebug.head._2.address.uint}]    ${bDebug(
        0
      )._1} ${bDebug(1)._1} ${bDebug(2)._1} ${bDebug(3)._1}"
    )
  }

  val cDebug = c.partitions.map { p =>
    val cMemArr = DFBits(32).X(32) init Vector.fill(32)(b0s)
    ...
    ifdf(p.ce) {
      ifdf(p.we) {
        sim.report(
          msg"    $cycle    wr    c_${p.idx}[${p.address.uint}]    ${p.d.uint}"
        )
        cMemArr(p.address.uint) := p.d
      }
      q := cMemArr(p.address.uint)
    }
    val cMemArrRef = DFBits(32).X(32) init cMatrix.getPartition(p.factor, p.idx)
  ...
  ifdf(cDebug.head._2.ce && !cDebug.head._2.we) {
    sim.report(
      msg"    $cycle    rd    c[${cDebug.head._2.address.uint}]    ${cDebug(
        0
      )._1} ${cDebug(1)._1} ${cDebug(2)._1} ${cDebug(3)._1} ${cDebug(
        4
      )._1} ${cDebug(5)._1} ${cDebug(6)._1} ${cDebug(7)._1}"
    )
  }
...
object matmulBlockApp extends App {
  ...
  top.printCodeString.compile.printCodeString.toFile(
    "matmulBlock/zedboard/matmul_ait/xilinx/HLS/loopback/solution1/impl/ip/hdl/vhdl/matmulBlock_moved.vhd"
  )
}
