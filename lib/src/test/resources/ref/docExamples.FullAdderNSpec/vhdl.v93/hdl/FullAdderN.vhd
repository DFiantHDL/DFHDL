...
architecture FullAdderN_arch of FullAdderN is
  ...
  signal adder_c_in   : std_logic_vector(3 downto 0);
  signal adder_c_out  : std_logic_vector(3 downto 0);
...
begin
  ...
  adder_0_a     <= a(0);
  adder_0_b     <= b(0);
  adder_0_c_in  <= adder_c_in(0);
  adder_c_in(1) <= adder_c_out(0);
  adder_1_a     <= a(1);
  adder_1_b     <= b(1);
  adder_1_c_in  <= adder_c_in(1);
  adder_c_in(2) <= adder_c_out(1);
  adder_2_a     <= a(2);
  adder_2_b     <= b(2);
  adder_2_c_in  <= adder_c_in(2);
  adder_c_in(3) <= adder_c_out(2);
  adder_3_a     <= a(3);
  adder_3_b     <= b(3);
  adder_3_c_in  <= adder_c_in(3);
  c_out         <= adder_c_out(3);
end FullAdderN_arch;
